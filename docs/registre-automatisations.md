...
| `claude-remote-watch/run.sh` | `*/10 * * * *` | 🟢 actif | démarre `claude-remote` si constaté à l'arrêt (jamais un restart aveugle) |
| `security-posture-watch/run.sh` | `35 6,18 * * *` | 🟢 actif | contrat PASS/FAIL/UNKNOWN/STALE -- une mesure impossible ne devient jamais un PASS |
...
- **claude-remote-watch (03/09, operator-requested: "je veux que la session reste intacte meme si je m'absente 1 mois")**: `/opt/aria-data/claude-remote-watch/run.sh`, VPS cron every 10 min. Third safety net behind the launcher's own restart loop (`/opt/aria/start-claude.sh`) and systemd (`Restart=always`, `StartLimitIntervalSec=0`). Covers only what those two cannot: systemd itself never restarted the unit. Starts a service found stopped -- never a blind `restart`, which would kill a live working session. Telegram alert on state CHANGE only, so a month-long outage cannot spam. Since 03/09 it also stops orphaned Remote Control hosts -- every `/rc` launched from an SSH connection spawns a host PARALLEL to the service (own login scope, own server, own run directory) that nothing supervises and that eats the permanent host's 32 slots, the real origin of the "Capacity 0/32" symptom. Deliberately narrow criterion: only a `--serve` process outside the service, with NO child process (so nobody is working in it) and older than 2h -- a host still carrying a session is never touched, whatever its age. Detail: `docs/HANDOFF_VPS_OPS.md` (2026.09.03).
- **security-posture-watch (03/09, operator directive: "le watchdog ne doit pas compter des problemes, il doit verifier que nous avons suffisamment de preuves pour pouvoir dire PASS")**: `/opt/aria-data/security-posture-watch/run.sh`, VPS cron twice daily. A thin scheduler over `scripts/security_posture_collect.py`; all the logic lives in the repo under pytest. Emits a four-state contract (PASS / FAIL / UNKNOWN / STALE) plus `posture.json` and `posture.md`. The invariant, covered by 18 negative-control tests: an absent measurement, a partial coverage pair, a missing expected check or an empty evidence set all yield UNKNOWN -- never PASS. Detail: `docs/HANDOFF_SECURITE.md` (2026.09.03).
