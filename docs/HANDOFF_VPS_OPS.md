...
[CODE] Subject : POST /internal/diagnose/wallet-sourcing -- safe alternative to a blocked docker exec
Date : 2026.08.14 / Problem : item #151 (observe the real impact of 3 cumulated candidate sources on wallet_scan_queue throughput) needed one manual trigger of `smart_money_leaderboard_discovery_cycle` between its normal 180min heartbeat passes, to see per-source counts without waiting hours. `docker exec aria-api python -c "..."` was blocked outright by the session's own security classifier, even though the call was read-diagnostic in intent -- a raw shell command can't be scoped to "read-only" by pattern alone, so a blanket permission rule to allow it would have opened the door to arbitrary code in the prod container, not just this one diagnostic.
Solution : new endpoint, same secret-gated pattern as `/internal/activate-heartbeat` but a DEDICATED secret (`internal_diagnostic_secret`/`INTERNAL_DIAGNOSTIC_SECRET`, never reused across admin routes -- least privilege, verified a leaked/known deploy secret is explicitly rejected here too). Runs the EXACT same operation as the real heartbeat task (`discover_and_enqueue_candidates`), on demand instead of waiting for its scheduled pass -- same real side-effects (idempotent enqueue), never a separate/parallel code path that could drift from the real cycle. Bounded to one predefined operation, not arbitrary code -- no shell access needed for this class of diagnostic going forward. `app/main.py`, `app/config.py`, `tests/test_internal_diagnostic_endpoint.py` (6 tests: fail-closed unconfigured, missing header, wrong secret, deploy secret explicitly rejected, correct secret returns the real result). Full backend suite green (299 passed). **Operator action needed**: generate and set `INTERNAL_DIAGNOSTIC_SECRET` in the VPS `.env`, then redeploy.
