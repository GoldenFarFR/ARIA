...
Signature de transparence sur toute réponse (« Response generated by ARIA (an autonomous AI
owned by GoldenFarFR) »), zéro em-dash. Commande opérateur `/github repair` (admin-gated)
**édite** un commentaire déjà posté (jamais un nouveau post). Invariant verrouillé par
`test_coherence`.

## Swap de test Sepolia — CODÉ, PAS ENCORE ARMÉ (09/07 ; préservé de CLAUDE.md le 24/07)
Décision opérateur explicite « swap réel sur Sepolia, actif de test » — `sepolia_wallet.send_test_swap_transaction`
(wrap WETH → approve → `exactInputSingle` Uniswap V3, trois transactions réellement signées) +
gate additif `ARIA_SEPOLIA_SWAP_ENABLED` (au-dessus des 3 gates Sepolia déjà actifs —
`ARIA_SEPOLIA_WALLET_ENABLED`/`ARIA_SEPOLIA_AUTONOMOUS_ENABLED`/ancrage configuré), plafond dur
`MAX_TEST_SWAP_WEI` (~0.002 ETH), montant fixe `TEST_SWAP_AMOUNT_WEI` jamais dimensionné par Kelly.
Câblé dans `run_autonomous_cycle` : tentative de swap indépendante de l'ancrage de décision sur
BUY, l'échec de l'un n'efface jamais le succès de l'autre. Porte sur une paire de TEST configurée
(`ARIA_SEPOLIA_SWAP_ROUTER`/`ARIA_SEPOLIA_SWAP_TOKEN_OUT`), **jamais** le token candidat réellement
analysé (inexistant sur Base Sepolia — chaîne différente de Base mainnet, aucun contrat en commun).
**Bloquant avant activation, jamais vérifié à ce jour** : confirmer qu'un routeur Uniswap V3 (ou
équivalent) a du bytecode déployé sur Base Sepolia et qu'une pool WETH/X a une liquidité réelle non
nulle, avant de renseigner les env vars et d'armer le gate. Le blocage d'origine noté (« pas d'accès
RPC sortant depuis une session cloud ») ne tient plus — toute session tourne désormais sur le VPS
avec accès réseau réel (cf. section Sepolia autonome ci-dessus).
