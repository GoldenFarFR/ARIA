...
|---|---|---|---|---|---|
| GeckoTerminal (Demo key) | 30 req/min documented, real sustained cap lower under load (walked back to 15/min after two 429 storms on 08/02, then a 5x live test on 04/08 tripped the adaptive circuit breaker within ~10s and stayed open — real sustained cap confirmed well under 75 req/min) | Confirmed (official docs, researched 07/19) — but the documented figure has never held under real sustained conditions in practice, walked back 3 separate times now | 4.0s (15/min) | 2.222s (27/min) would be the naive 90%-of-documented target, but real conditions have never supported it — 15/min is the last empirically-survived value | **04/08: widened 5x (4.0s→0.8s) as an operator-directed live test, REVERTED minutes later** — the adaptive per-provider circuit breaker (3 consecutive failures) tripped almost immediately and stayed open for the full observation window (sustained, not isolated), and the operator flagged the real, already-lived Cloudflare-block risk (08/01) as a reason not to keep pushing at that rate. Back to 15/min. The burst-controlled empirical measurement this file's own doctrine calls for (task #41, still open) remains the right way to find the real ceiling — a bounded, monitored test, not a permanent throttle change tested live. |
...
| CoinMarketCap Pro (Basic) | 50 req/min, 15000 credits/month | **Confirmed live** via `/v1/key/info` on the real configured key | 1.5s (40/min) + proactive monthly-quota guard (see below) | 1.333s (45/min) | **Tightened 08/02** (explicit operator decision): 18 HTTP 500 errors (not 429 -- server error, not our own throughput being exceeded) observed on `/v1/dex/token/pools` over ~4 min. A precautionary reduction, not proof that throughput was the cause -- to revisit if the 500s persist after this change. **12/08**: the per-minute throttle alone missed a real gap -- the real key was found at 14569/15000 monthly credits used (97%) with zero code-level protection against the MONTHLY quota itself. `coinmarketcap_quota_guard.py` now proactively polls `/v1/key/info` (free, confirmed zero credit cost) every 10 minutes and auto-suspends new calls under 5% remaining, same self-healing doctrine as `goplus_quota_suspension.py` but proactive (real number known and free to check) rather than reactive (waiting for a 429/4029 to arm). |
| CoinGecko (Demo, `/simple/price`) | 100 req/min, 10,000 credits/month, flat 1 credit/call (confirmed: CoinGecko's own "Flat Credit Model" doc, unlike Mobula's variable per-endpoint cost) | Confirmed (2 independent official sources) | 0.667s (90/min) | 0.667s (90/min) | Speed-up already done. **12/08 (backlog #111): real gap found and fixed** — the per-minute throttle never protected the MONTHLY credit budget, and CoinGecko is used far more widely than the OHLCV cascade alone (`market_sentiment.py`, `momentum_entry.py`, `paper_trader_risk.py`, `arena_signal.py`, 5 more real call sites). `/api/v3/key` (the live balance-check endpoint) returned HTTP 10005 "limited to PRO API subscribers" when tested live against the real configured Demo key — not usable, same constraint as Mobula/Dune. `coingecko.py` now tracks calls this calendar month locally (flat 1 credit/call), capped at 9,500 (95% of 10,000), fails closed once reached. |
| Mobula | 1 req/s, 10,000 credits/month | Confirmed (official docs) | 1.05s (95.2%) | 1.111s (0.9/s) | Slight slowdown. **12/08 (backlog #111): real gap found and fixed** — the per-second throttle never protected the MONTHLY credit budget, and `/2/token/ohlcv-history` (the only endpoint this client calls) costs 5 credits/call, not 1 (docs.mobula.io/pricing, "Credit Consumption Per API Call"). Operator confirmed the real account dashboard showed 42.5K/10K credits used (425% overage), API still "Operational" — Mobula does not cut off server-side. No live balance-check endpoint usable without extra account config (`/api/2/usage` needs `customerId`/`organizationId`, neither set in prod). `mobula.py` now tracks credits spent this calendar month locally, capped at 9,500 (95% of 10,000), fails closed on this provider only once reached. |

## Tier 2 — x402 and secondary data providers

| Service | Real throughput (source) | Confidence | Current throttle | 90% target | Action |
|---|---|---|---|---|---|
| Dune Execute SQL (Free) | 15 req/min (low, binding limit) + 40/min (high limit, separate counter); separately, 2,500 credits/month | Confirmed (official docs) | 4.44s (13.5/min) | 4.44s (13.5/min) | **12/08 (backlog #111): real gap found and fixed** — the per-minute throttle never protected the MONTHLY credit budget. No live balance-check endpoint confirmed in Dune's public docs (unlike CoinMarketCap). Only `execute_sql` (`POST /v1/sql/execute`) spends a credit — `get_execution_status`/`get_execution_result` are documented free and were never counted. `dune.py` now tracks executions this calendar month locally (a proxy for credits — real per-execution cost can vary with query complexity, no fixed-credits-per-call doc found), capped at 2,375 (95% of 2,500), fails closed on this provider only once reached. |
...
| DexPaprika | 15/min keyless, 30/min free registered key, 50k/300k monthly credits respectively (official comparison table, operator-verified live screenshot 17/08) | Confirmed (official site) for the documented figure — but a burst test the same day (25 keyless + 107 keyed requests, 132 total, ~30s) survived well past both documented per-minute figures before the first 429, so the real burst tolerance is much higher than advertised; sustained long-run throughput NOT isolated (same IP for both legs of the test, no cooldown measured) | 1.2s (50/min) — unchanged after adding the key, deliberately conservative (operator: "throttle prudent qui satisfait déjà bien le pipeline shadow"), already above the documented 30/min keyed figure | Not recalculated — current throttle already exceeds the documented ceiling, no need to push further for current shadow-only usage | **17/08**: `DEXPAPRIKA_API_KEY` (free tier, no card) added to `vanguard/backend/.env` — the code seam (`services/dexpaprika.py::_auth_headers()`) already existed since 04/08, nothing to wire. Also found live: `shadow_persistent.py` (the Solana/Robinhood shadow, runs outside Docker) had NO access to any prod `.env` variable at all (not just this key — `COINGECKO_DEMO_API_KEY` too), so it was hitting GeckoTerminal unauthenticated while prod used the authenticated throttle; fixed via `load_dotenv()` at the top of the script, before any `aria_core` import that instantiates a module-level client. x402 micropayments (0.001$/call, no key/subscription) announced by DexPaprika as "shipping this quarter" — not live yet, banked for a future re-check, never implemented on a promise. |
