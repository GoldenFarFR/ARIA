...
| CoinMarketCap Pro (Basic) | 50 req/min | **Confirmé en direct** via `/v1/key/info` sur la vraie clé configurée | 1,5s (40/min) | 1,333s (45/min) | **Resserré 02/08** (décision opérateur explicite) : 18 erreurs HTTP 500 (pas 429 -- erreur serveur, pas notre propre débit dépassé) observées sur `/v1/dex/token/pools` en ~4 min. Baisse de précaution, pas une preuve que le débit en était la cause -- à revoir si les 500 persistent après ce changement. |
