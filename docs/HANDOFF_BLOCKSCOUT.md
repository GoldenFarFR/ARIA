...
[CODE] Subject : duplicate calls on the same contract -- plumbing fix + shared short-TTL cache on the singleton
...
`skills/acp_onchain_scan.py`/`services/blockscout.py`/`tests/test_acp_onchain_dev_behavior.py` (new, 3 tests)/`tests/test_blockscout_client.py` (+5 cache tests). Full suite green (9906 passed, 17 skipped). Not deployed yet.
