...
[CODE] Sujet : appels dupliqués sur le même contrat -- fix de plomberie + cache court-TTL partagé sur le singleton
Date : 2026.08.11 / Probleme : backlog #93 visait un "historique persistant Blockscout" façon candle_history, mais un audit Explore (recherche read-only, call-graph réel) a trouvé un gain différent et plus immédiat : (1) un vrai bug de plomberie -- `acp_onchain_scan._resolve_dev_behavior` (crible VC) n'a jamais reçu le paramètre `holders=` ajouté le 28/07 à `gather_dev_wallet_facts` pour justement éviter ce cas, alors que `_resolve_insider_wallets` juste en dessous le réutilise déjà correctement -- refaisant `get_token_holders` en double pour le MÊME contrat dans la MÊME évaluation VC ; (2) `get_address_info` appelé séparément par ces deux mêmes fonctions pour le même token_address ; (3) aucun cache partagé entre les appelants indépendants du singleton `blockscout_client` (momentum_entry, VC crible, `paper_trader_risk`'s recheck d'invalidation à 15min après l'entrée) alors que `_check_holder_concentration` a son propre cache 300s local et `holder_concentration_cache.py` son propre cache-verdict 7 jours, disjoints de tout le reste.
Solution : (1) fix direct -- `_resolve_dev_behavior(ctx, token_address, holders=None)` transmet maintenant `holders` à `gather_dev_wallet_facts`, zéro appel réseau économisé de façon garantie sur ce call-site. (2) cache court-TTL (60s, même doctrine que `momentum_entry._security_cache`/`_pair_snapshot_cache`) directement dans `BlockscoutClient` (`_cache_get`/`_cache_set`, clé `(méthode, adresse.lower())`) pour les 6 méthodes prouvées dupliquées ou dupliquables : `get_address_info`, `get_token_metadata`, `get_token_holders`, `check_contract_flags`, `get_verified_source`, `read_owner`. Couvre TOUS les appelants du singleton sans les modifier un par un. **Jamais un résultat `available=False` n'est mis en cache** (piège trouvé en testant : ça cassait le disjoncteur circuit-breaker existant, qui a besoin de VOIR chaque échec réel consécutif pour s'ouvrir -- même doctrine que `holder_concentration_cache.py`, juste appliquée ici à un TTL court plutôt que 7 jours).
`skills/acp_onchain_scan.py`/`services/blockscout.py`/`tests/test_acp_onchain_dev_behavior.py` (nouveau, 3 tests)/`tests/test_blockscout_client.py` (+5 tests cache). Suite complète verte (9906 passed, 17 skipped). Pas encore déployé.
