...
[DEPLOYE] Sujet : circuit breaker never fired + proactive idle-close for the newHeads keepalive (25/08, same-day follow-up)
Date : 2026.08.25 / Probleme : operator screenshot showed Robinhood overshooting its whole daily quota (286,922 RU vs the 200k/day cap from the entry above) after only ~9h of a 24h day, while Base consumed almost nothing -- yet `grep -c "CIRCUIT BREAKER OPEN"` on the prod log returned 0. Root cause: `_check_budget_circuit_breaker`'s trigger was `if not spendable and self._pools and not self._breaker_open` -- the `and self._pools` guard meant the breaker only ever fired while at least one pool was actively tracked. The newHeads keepalive (the entry above's own fix) runs precisely when `self._pools` is EMPTY -- exactly the state this guard skipped -- so on Robinhood's ~100ms blocks the single biggest cost was never something this breaker could touch.
Solution : (1) breaker trigger now `if not spendable and not self._breaker_open` (drops the `_pools` guard entirely), evicts whatever pools exist (zero, one, or many) and closes the keepalive too via `_resubscribe()` (now also checks `self._breaker_open` before reopening newHeads). (2) Separately, `_resubscribe()` only re-evaluates idle state when `add_pool()`/`remove_pool()` calls it -- with nothing happening at all (the exact idle state the keepalive exists for), nothing would ever trigger that re-evaluation on its own, so the keepalive could run all day even under budget. New `_check_idle_newheads()` (called every `_BREAKER_CHECK_INTERVAL_SECONDS` tick from the same `_breaker_loop`) proactively closes the keepalive once `_pools` has been empty for `_IDLE_NEWHEADS_CLOSE_SECONDS = 120.0` -- never waiting for the daily budget to blow first. Verified live: RU counters for base/robinhood/solana held exactly flat (2320/286922/28018) across a 45s window with Base/Robinhood discovery actively running, confirming zero residual Chainstack cost from either fix.
`services/evm_swap_ws.py` (`_check_budget_circuit_breaker`, `_resubscribe`, new `_check_idle_newheads`) / `tests/test_evm_swap_ws.py` (+7 tests: breaker fires with nothing tracked, breaker closes the keepalive, resubscribe never reopens newHeads while breaker open, idle-close no-op before/after the window, resubscribe reopens promptly on a fresh pool, idle tracking clears once a pool returns) -- 62 tests in the file, 442 across the targeted suite (a5d23161/12637a6d/81c9d185).

------------------------------------------------------------

[DEPLOYE] Sujet : pumpfun_bonding_ws.py never counted its own Chainstack RU spend -- internal budget blind to the single biggest real Solana cost
Date : 2026.08.27 / Probleme : operator's real 7-day Chainstack dashboard showed 2,899,990 total requests (peaks 500-600k/day), dominated by `accountNotification` (~1.8-2M) then `getMultipleAccounts` (~600-700k) -- while ARIA's own internal `chainstack_ru_budget` counter for Solana showed only 115,823/175,000 (66%) used. Grepped every Solana/Base/Robinhood feed handler: `pumpfun_curve_tracker.py` (`getMultipleAccounts`) and `onchain_pool_discovery.py`/`evm_swap_ws.py` (`_handle_notification`) all correctly call `chainstack_ru_budget.record_usage_fast`. `services/pumpfun_bonding_ws.py`'s `_apply_notification()` -- the handler for every `accountNotification` push, i.e. the single dominant billing category on the real dashboard -- was the sole exception, never calling it at all. This fully explains the gap between the internal counter and the real bill.
Solution : `chainstack_ru_budget.record_usage_fast("solana", 1)` added as the first action inside `_apply_notification()`, before any decode, same "count on arrival" doctrine already documented in `solana_gateway.py`. Note: this fix makes the internal budget honest, it does NOT by itself reduce real spend -- the actual volume reduction that day came from the operator disabling `ARIA_AGENT_WALLET_PILOT_ENABLED`/`ARIA_SOLANA_TRADE_PILOT_ENABLED` (real-capital pilot, unrelated RU category, see `docs/HANDOFF_SOLANA_TRADE_PILOT.md`) and separately deciding whether to keep the Solana shadow sourcing itself running (the actual source of the `accountNotification` volume, still active as of this entry). 11456/11456 tests pass, no other file touched by this fix.
`services/pumpfun_bonding_ws.py` (`_apply_notification`) / `services/evm_swap_ws.py` (comment fix) (43c9fa97).

------------------------------------------------------------

[DEPLOYE] Sujet : dedicated on/off gate for the curve_tracker polling node -- operator wanted a node-level switch, not shadow_pause
Date : 2026.08.30 / Probleme : operator noticed live Chainstack dashboard traffic (~3030 req/h, 97% success) on the dedicated polling node and asked why, believing a gate was already off. `ARIA_SOLANA_TRADE_PILOT_ENABLED=false` was indeed off, but that only gates real execution -- `pumpfun_curve_tracker.py`'s batched `getMultipleAccounts` poll (feeding `solana_late_bonding_shadow`) kept running regardless, and the only broader lever (`shadow_pause`/`/offshadow`) would have also frozen Robinhood/Base tracking, an unwanted side effect. Confirmed `ARIA_SOLANA_RPC_HTTP` and `ARIA_SOLANA_RPC_HTTP_POLLING` resolve to the identical node, so this is indeed the node shown on the dashboard.
Solution : new `ARIA_SOLANA_CURVE_TRACKER_NODE_ENABLED` gate (default ON, fail-open -- observation only, zero capital), checked at the top of `poll_due()` before any RPC spend. Set to `false` in prod `.env`, `aria-shadow-persistent` service restarted. Verified live: `CURVE TRACKER shadow` log line now reads `credits=0` every cycle (was driving the ~3030 req/h). Residual Solana RU budget movement (~200/cycle) comes from other legitimate consumers on the same node (rent recovery, late-bonding tracking) -- correctly left untouched. `services/pumpfun_curve_tracker.py`, `tests/test_pumpfun_curve_tracker.py` (+2 tests), `tests/test_coherence.py`, `memory/capability_state.py` (a0cb4c4f, push to origin pending -- git proxy timing out, retry later).
