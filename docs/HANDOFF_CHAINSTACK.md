...
- ARIA's actual current Chainstack plan tier and configured RPS/RU usage were **not verified in this session** — this file documents Chainstack's platform-wide structure, not ARIA's live account state; check `console.chainstack.com` before citing a specific headroom number.
