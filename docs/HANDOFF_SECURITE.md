...
[ETAT ACTUEL] Subject : GoPlus Security watch #128 -- Robinhood Chain chain_id mapping confirmed present in code, never empirically validated by real traffic
Date : 2026.08.18 / Problem : GoPlus announced official Token/Transaction Security coverage for Robinhood Chain on X -- flagged TRAITER by the goplus-security-watch judge pass, asking to verify ARIA's own GoPlus client actually uses the right chain_id, not just cited by GoPlus's own partner list.
Solution : confirmed `_DEXSCREENER_TO_GOPLUS_CHAIN_ID["robinhood"] = "4663"` (momentum_entry.py, added #308/16/08) is present and wired into every honeypot check path (`_add_candidate`, `_check_watchlist_candidate`). Attempted a real live GoPlus API call (`/supported_chains`) to confirm 4663 is genuinely listed server-side -- blocked: GoPlus's own quota is currently suspended (`goplus_quota_suspension_state`, 4th consecutive failure, backoff until 2026-08-19T12:27Z, `goplus_quota_suspension.py`'s existing self-healing bypass, not a new incident). Checked the DB directly instead of guessing: `goplus_watchlist` has ZERO rows with `chain='robinhood'` and `momentum_scan_log` has ZERO rows with `chain='robinhood'` at all -- Robinhood Chain momentum sourcing has never actually produced a single real candidate in prod, so this mapping has never been exercised by real traffic either way. Not fixed because there is nothing broken to fix yet -- the code path is correct as far as static inspection goes, but genuinely unverified. Next concrete step (banked, not urgent -- nothing here crosses the 48h bar): once the GoPlus quota backoff clears (~19/08 12:27 UTC), run one real `token_security/4663` call against a known Robinhood Chain contract (e.g. one of the Stock Tokens from #309) before trusting this coverage under live capital. Cross-referenced with #308 (Blockscout holder-concentration, already confirmed live on Robinhood Chain) -- the two pillars are NOT symmetrically verified yet.
...
--------------------------------------------------------------------------
[CODE] Sujet    : column migrations raced between processes
