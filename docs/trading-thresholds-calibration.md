...
|---|---|---|---|---|
| `_MIN_LIQUIDITY_USD` | momentum_entry.py:160 | 25 000 $ | **Abaissé 31/07** (était 50 000 $ depuis le 21/07), décision opérateur explicite, même mouvement que le retrait du plancher R/R swing ci-dessous -- philosophie assumée : plus de candidats à l'entrée, le tri se fait au niveau LLM/R-R visible plutôt qu'à la porte. `_MIN_LIQUIDITY_USD_FEAR` volontairement PAS rescalé (reste 100 000 $, x4 au lieu du x2 calibré le 20-21/07 -- effet secondaire connu et accepté, pas perdu de vue) | Le funnel montre ce gate comme goulot dominant sur plusieurs cycles consécutifs |
...
| `_RR_MIN_FOR_DIRECT_BUY` | momentum_entry.py:190 | 2.0 | 18/07, décision opérateur "plus sélectif". **Scalping-only depuis le 31/07** -- swing (mode != "scalping") n'utilise plus ce plancher du tout, voir la note ci-dessous | Taux de faux positifs/négatifs observé sur les achats directs vs LLM (scalping) |
| `_RR_AMBIGUOUS_FLOOR` | momentum_entry.py:191 | 1.0 | Inchangé depuis l'origine. **Scalping-only depuis le 31/07**, même note que ci-dessus | — |
| *(retrait) plancher R/R swing* | momentum_entry.py:3302-3376 | aucun | **31/07, décision opérateur explicite ("enlève le r/r minimum ... sur swing")** -- swing n'a plus aucun plancher R/R : ni achat direct sans LLM, ni HOLD pur pour R/R faible/négatif. Chaque setup swing (golden pocket + RSI formé) passe désormais toujours par `_llm_confirm_and_gate` (confirmation + sécurité fusionnées), quel que soit le R/R. Même rationale que le retrait du plancher R/R des ordres limite le même jour (Item #252) : l'entrée ne borne pas le potentiel de sortie (trailing stop). Scalping garde `_RR_MIN_FOR_DIRECT_BUY`/`_RR_AMBIGUOUS_FLOOR` inchangés | Si le taux de rejet LLM sur R/R faible/négatif s'avère trop permissif ou trop strict une fois assez de données accumulées |
| `_ALIGN_SCORE_MIN_FOR_DIRECT_BUY` | momentum_entry.py:196 | 2 | 18/07, même décision que `_RR_MIN_FOR_DIRECT_BUY` | Idem |
| `MAX_VOLUME_TO_LIQUIDITY_RATIO` | momentum_entry.py:212 | 20.0x | 17/07, incident réel BRIAN (-17,9%, -8 962$, ratio ~91x observé). **Scalping exempté depuis le 02/08**, décision opérateur explicite : une hausse causée par du wash-trading n'est pas intrinsèquement nuisible à une stratégie de sortie rapide (le scalping peut suivre le mouvement et sortir avant un effondrement post-pump) — le risque que ce garde-fou protège réellement (tenir jusqu'à l'effondrement) est spécifique à une position tenue longtemps. Toujours actif pour swing/megacap (`mode="standard"`, même exposition tenue-longue) et vc (`skills/safety_screen.py`, site séparé, non touché). Exemption appliquée aux DEUX sites — entrée (`momentum_entry.py::evaluate_hard_gates`, `mode != "scalping"`) ET re-vérification continue en cours de détention (`paper_trader_risk.py::rescan_open_position`, `position.get("mode") != "scalping"`) | Un nouveau cas confirmé de wash-trading sous ce seuil sur swing/megacap/vc, ou une perte réelle en scalping directement attribuable à un pump wash-trading suivi d'un effondrement avant sortie |
...
| `_MIN_VOLUME_24H_USD` | momentum_entry.py:425 | 500 $ | **Gate retiré 30/07, Item #246, décision opérateur explicite ("supprime le")** -- la constante reste (encore utilisée par le pré-filtre de découverte Birdeye, `_refresh_birdeye_cache`), mais elle ne bloque plus aucun achat dans `evaluate_hard_gates`. Historique avant retrait : abaissé 5000→1000 le 20/07, étendu 1000→500 le 21/07 sur 2 diagnostics funnel successifs | — |
| `_MIN_VOLUME_TO_LIQUIDITY_RATIO` | momentum_entry.py:445 | 1% | Même retrait 30/07, Item #246 -- constante conservée pour Birdeye uniquement, plus aucun rôle de gate. 21/07, coordonné à l'origine avec `_MIN_LIQUIDITY_USD` (jonction exacte à 500$) | — |
...
| ~~`_STALENESS_DAYS_THRESHOLD`/`_STALENESS_MAX_DAYS`/`_STALENESS_MAX_PENALTY_PCT`/`_STALENESS_WAIVER_POSTING_CADENCE`~~ | — | — | **Retiré 30/07, Item #242** — décision opérateur explicite : ARIA ne doit jamais appliquer de malus à un jeton pour faible market cap ou statut bonding ; la décote "déclin organique" (#161/#162) faisait exactement ça. Mécanisme entièrement supprimé de `bonding_entry.py`, plus une constante à recalibrer | — |
| `_HOLDER_CONCENTRATION_UNINFORMATIVE_SCORE_FRACTION` | bonding_entry.py:222 | 0.5 (neutre, 7.5/15) | 24/07, valeur neutre d'origine. Abaissée à 0.2 (quasi-nul) le 28/07 par une passe de recherche, puis **REVERTÉE 30/07, Item #242** (même directive opérateur ci-dessus — sous le seuil de 50 holders, un token est presque par définition petit/récent ; le score bas pour ça était exactement le pattern interdit) | — |
