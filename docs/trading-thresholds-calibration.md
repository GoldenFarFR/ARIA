...
| `MAX_VOLUME_TO_LIQUIDITY_RATIO` | momentum_entry.py:212 | 20.0x | 17/07, incident réel BRIAN (-17,9%, -8 962$, ratio ~91x observé). **Scalping exempté depuis le 02/08**, décision opérateur explicite : une hausse causée par du wash-trading n'est pas intrinsèquement nuisible à une stratégie de sortie rapide (le scalping peut suivre le mouvement et sortir avant un effondrement post-pump) — le risque que ce garde-fou protège réellement (tenir jusqu'à l'effondrement) est spécifique à une position tenue longtemps. Toujours actif pour swing (`mode="standard"`, même exposition tenue-longue) et vc (`skills/safety_screen.py`, site séparé, non touché). Exemption appliquée aux DEUX sites — entrée (`momentum_entry.py::evaluate_hard_gates`, `mode != "scalping"`) ET re-vérification continue en cours de détention (`paper_trader_risk.py::rescan_open_position`, `position.get("mode") != "scalping"`) | Un nouveau cas confirmé de wash-trading sous ce seuil sur swing/vc, ou une perte réelle en scalping directement attribuable à un pump wash-trading suivi d'un effondrement avant sortie |
