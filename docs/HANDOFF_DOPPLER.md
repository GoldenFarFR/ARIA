...
[DEPLOYE] Sujet: Doppler became the momentum sourcing pipeline's 3rd discovery source (>90% of new Base pools) -- corrects the stale "never an autonomous decision" status below
Date: 2026.08.25 (file-staleness-watch review, real change dated 2026.08.14) / Probleme: the 24/07 CURRENT STATE entry said "no bonding_entry.evaluate_bonding_entry equivalent built for Doppler -- positions opened only via operator-forced test buys, never an autonomous ARIA decision." Stale: discover_recent_pools() has been wired as the 3rd source in momentum_entry.py since 14/08.
Solution: services.doppler.discover_recent_pools() scans every Uniswap v4 PoolManager Initialize event in the cursor's current window, identifies the candidate token (the pair side absent from reference_tokens_excluded), and feeds the same honeypot pipeline (_check_honeypot/goplus_watchlist) as the other sources -- the rest of the guardrails (golden pocket/RSI/RVOL/established profile) are still applied by each pocket at buy-decision time, not by this discovery step. Still true: no bonding_entry.evaluate_bonding_entry dedicated to Doppler, entry sizing/gating goes through the standard momentum pipeline, not a Doppler-specific path -- `packages/aria-core/src/aria_core/momentum_entry.py`, `services/doppler.py`.
