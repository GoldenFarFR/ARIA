...
| **EMA/MACD réels** (#68) | skill prêt, non câblé | `skills/indicators.py` (`ema_series`/`macd_series`, déterministe, testé) | Écart réel comblé : `CLAUDE.md` annonçait ces indicateurs depuis longtemps, ils n'étaient calculés nulle part. Point d'ancrage naturel pour le câblage : `acp_onchain_scan.py:720` (`ctx.ta = compute_levels(...)`) — pipeline de scan réel déjà en prod, à ne pas modifier sans validation (même prudence que #11/#59). |
| **Setup « golden pocket + divergence RSI »** (#69, trouvé le 10/07) | skill complet, non câblé | `skills/entry_signals.py` (`detect_entry`, 171 lignes, 10 tests — mais absent de la CI avant ce commit) | Capacité DÉJÀ construite et testée mais totalement invisible du rapport `/vc` en prod : ne recoupe pas `ta_levels.suggest_entry_zone` (déjà câblé, `ctx.ta_entry`) qui est générique (support/résistance le plus proche, toujours renvoyé si les niveaux existent) — `detect_entry` est un signal plus rare et plus qualitatif (`present=True` seulement si prix dans la zone Fibonacci 0,618–0,786 ET divergence haussière RSI coïncident), avec R/R dérivé des niveaux réels. Point d'ancrage naturel : `acp_onchain_scan.py:721` (juste après `ctx.ta_entry = suggest_entry_zone(ctx.ta)`), en complément — pas en remplacement — du signal générique existant. Décision de l'ajouter à la thèse `/vc` (rapport premium déjà approuvé) laissée à l'opérateur, même prudence que #11/#59/#68. |
