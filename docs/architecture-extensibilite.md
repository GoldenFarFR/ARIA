...
| **Découverte multi-launchpad (bonding + gradués + directs)** (10/07) | registre d'adaptateurs + 2 pipelines | `services/launchpad_discovery.py` (catégories `bonding`/`direct`/`unknown`, réutilise `mint_authority.is_bonding_launchpad` comme SEULE source de vérité) → `skills/bonding_screen.py` (pendant de `safety_screen` sans exiger de paire DEX) → `skills/bonding_absorber.py` (`screened_pool` sous `network="base-bonding"`) ; tâche heartbeat `bonding_discovery_cycle` (180min, gate OFF `ARIA_BONDING_DISCOVERY_ENABLED`) | `screened_pool.list_pool/count_pool/draw_lottery` gagnent un paramètre `network="base"` (défaut inchangé) pour ISOLER la niche bonding (15%) du pool 85% VC — jamais mélangés, `weekly_training.draw_lottery` reste 100% pool VC. `services/clanker.py` (nouveau client, même patron que `virtuals.py`) + `virtuals.fetch_graduated` (tokens ayant gradué, liquidité DEX réelle) alimentent le pipeline STANDARD (`token_absorber.absorb`, pool 85%), pas la niche. Launchpads sans diligence faite (Flaunch/Zora ont un client `discover=None` documenté ; Bankr/Ape.store/Mint.club sont des seams `knowledge/launchpads.yaml` sans adresse) : jamais de client fabriqué sur hypothèse (doctrine « profondeur proportionnelle à l'enjeu »). Un launchpad en échec n'arrête jamais les autres (best-effort par adaptateur). Clanker `/api/tokens` a renvoyé HTTP 403 en test cloud (probable anti-bot générique) — à reconfirmer depuis le VPS avant activation du gate. |
...
| **Scorecard « feu vert argent réel »** (#70, livré le 10/07) | skill + commande admin | `skills/real_money_readiness.py` → `/feuvert` Telegram (admin-only) | Mesure objectivement les cases pré-engagées de `docs/protocole-argent-reel.md` depuis le vrai journal `vc_predictions` — jamais un jugement subjectif. 3 cases calculables aujourd'hui (échantillon+étalement, intégrité structurelle, robustesse anti-chance), le reste (benchmark hold-ETH, vérif a posteriori des AVOID, calibration du juge) reste honnêtement `unknown` tant que la donnée manque — jamais transformé en `ok` par optimisme. **25/08** : la case "feu vert avocat" a été retirée du barème (décision opérateur), `skills/real_money_readiness.py` à vérifier/mettre à jour en conséquence si elle y était encore comptée. Seam pour compléter plus tard : un client de prix ETH/USD aligné sur les fenêtres réelles d'entrée/sortie (benchmark) et un module type `pump_dump_autopsy` appliqué aux AVOID (vérif a posteriori). |
