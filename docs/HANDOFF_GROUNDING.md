...
[CODE] Sujet    : Generic block explorer accepted as "official website" (conviction_research)
Date : 2026.08.12  /  Probleme : manual simulation on the 6 already-purchased VC/bonding candidates (operator request, "avant de coder simule les candidat elligible") found a real false positive on INF (bonding candidate): `ethplorer.io` (a generic block explorer) was missing from `_SOCIAL_OR_EXPLORER_DOMAINS`, so `_extract_website()` accepted it as the project's "official website" -- worse, the address shown on that explorer page was a DIFFERENT contract than the scanned token, `_contract_mentioned` correctly flagged `contract_corroborated=False`, but the explorer link and its scraped content still fed a positive `potential_score` (6.0/10), pushing the composite score just above the buy threshold on a signal that was never a real project website.
Solution : `_SOCIAL_OR_EXPLORER_DOMAINS` extended with `ethplorer.io` + 8 other common generic explorers (`blockscout.com`, `bscscan.com`, `polygonscan.com`, `arbiscan.io`, `snowtrace.io`, `ftmscan.com`, `tronscan.org`, `moonscan.io`), same family as `etherscan.io`/`basescan.org`/`solscan.io` already excluded. Regression tests lock in the real case (direct extraction + end-to-end via `research_project_potential`, `website_url` stays `None` on a single explorer-type Tavily result) -- conviction_research.py, test_conviction_research.py (2 new tests). 1st of 3 bugs found by the simulation (bug #2 X 401 on 4/6 candidates, bug #3 no "detective mode" deep search for bonding when Virtuals declares no link -- both still open).
