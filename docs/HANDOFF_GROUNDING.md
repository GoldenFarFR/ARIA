...
[CODE] Sujet    : Explorateur de blocs générique accepté comme "site officiel" (conviction_research)
Date : 2026.08.12  /  Probleme : simulation manuelle sur les 6 candidats VC/bonding déjà achetés (demande opérateur, "avant de coder simule les candidat elligible") a trouvé un vrai faux positif sur INF (candidat bonding) : `ethplorer.io` (explorateur de blocs générique) était absent de `_SOCIAL_OR_EXPLORER_DOMAINS`, donc `_extract_website()` l'a accepté comme "site officiel" du projet -- pire, l'adresse affichée sur cette page d'explorateur était un contrat DIFFÉRENT du token scanné, `_contract_mentioned` a bien détecté `contract_corroborated=False`, mais le lien d'explorateur et son contenu scrapé alimentaient quand même un `potential_score` positif (6.0/10), poussant le score composite juste au-dessus du seuil d'achat sur un signal qui n'a jamais été un vrai site de projet.
Solution : `_SOCIAL_OR_EXPLORER_DOMAINS` étendue avec `ethplorer.io` + 8 autres explorateurs génériques courants (`blockscout.com`, `bscscan.com`, `polygonscan.com`, `arbiscan.io`, `snowtrace.io`, `ftmscan.com`, `tronscan.org`, `moonscan.io`), même famille que `etherscan.io`/`basescan.org`/`solscan.io` déjà exclus. Tests de régression verrouillant le cas réel (extraction directe + bout-en-bout via `research_project_potential`, `website_url` reste `None` sur un résultat Tavily unique de type explorateur) -- conviction_research.py, test_conviction_research.py (2 nouveaux tests). 1er des 3 bugs trouvés par la simulation (bugs #2 X 401 sur 4/6 candidats, #3 absence de recherche "détective" en bonding quand Virtuals ne déclare aucun lien -- encore à traiter).
