"""Base momentum shadow (23/08) -- pure read+log, never a trigger, never
touching paper_trader/paper_pause_state (see module docstring). Mirrors
test_robinhood_pump_shadow.py's isolated-tmp-db + detect/measure state-machine
test pattern (the GeckoTerminal client is always injected/mocked, never a
real network call, same doctrine as test_geckoterminal_client.py)."""
...
async def _rows():
    async with aiosqlite.connect(shadow._db_path()) as db:
        ...
        cur = await db.execute("SELECT * FROM base_momentum_shadow_log")
...
class FakeClient:
    ...
    async def get_pool_snapshot(self, pool_address, *, network="base"):
...
@pytest.mark.asyncio
async def test_record_signals_persists_pool_age_at_entry():
    """23/08 -- the whole reason this module exists: the operator's original
    question ("trouve la meilleure fenetre de trading") needs this column
    queryable, not recomputed from pool_created_at/detected_at by hand."""
    created = datetime.now(timezone.utc) - timedelta(minutes=4.0)
    logged = await shadow.record_signals([_pool(m5=40.0, pool_created_at=created)], chain=CHAIN)
    assert logged == 1
    rows = await _rows()
    assert rows[0]["pool_age_at_entry_minutes"] == pytest.approx(4.0, abs=0.05)
...
async def test_record_signals_relogs_after_previous_signal_closed():
    ...
    async with aiosqlite.connect(shadow._db_path()) as db:
        await db.execute("UPDATE base_momentum_shadow_log SET status = 'closed'")
        await db.commit()
...
async def test_snapshot_fallback_unknown_liquidity_backfilled_from_dexpaprika_when_gecko_empty(monkeypatch):
    ...
    async def fake_get_pool_reserve_usd(pool_address, *, network="base"):
        return 2062.5
...
async def test_snapshot_fallback_skips_dexscreener_without_a_token_address(monkeypatch):
    async def unexpected_call(contract, *, chain="base"):
...
async def test_evaluate_old_row_awaiting_h2_never_starves_a_younger_rows_m15():
    """Same real live bug as solana_pump_shadow.py's twin test (17/08): a
    row already measured on m15+h1 (just waiting on h2) must never starve a
    younger row with a genuinely due m15, even with a small limit."""
    ...
    async with aiosqlite.connect(shadow._db_path()) as db:
        await db.execute(
            "UPDATE base_momentum_shadow_log SET forward_price_m15 = 1.1, forward_pct_m15 = 10.0, "
...
# --- run_cycle --------------------------------------------------------
# 23/08 -- sourced via dexpaprika.get_trending_pools (module-level function,
# not a client method -- see module docstring's sourcing note), distinct
# from the Robinhood twin's now-dormant GeckoTerminalClient.get_trending_pools
# path. The ``client`` param still exists for evaluate_open_signals/
# advance_exit_simulation below (pricing already-open rows).


@pytest.mark.asyncio
async def test_run_cycle_fetches_logs_and_measures(monkeypatch):
    from aria_core.services.geckoterminal import TrendingPoolsResult

    calls = []

    async def fake_get_trending_pools(network, *, limit=25):
        calls.append((network, limit))
        return TrendingPoolsResult(pools=[_pool(m5=30.0)], available=True, error=None)

    monkeypatch.setattr(shadow.dexpaprika, "get_trending_pools", fake_get_trending_pools)
    client = FakeClient({})
    result = await shadow.run_cycle(client, network=CHAIN)
    assert result["fetched_pools"] == 1
    assert result["signals_logged"] == 1
    assert calls == [("base", 25)]


@pytest.mark.asyncio
async def test_run_cycle_handles_unavailable_trending_pools(monkeypatch):
    from aria_core.services.geckoterminal import TrendingPoolsResult
...
async def test_summary_computes_win_rate_only_over_closed_rows():
    async with aiosqlite.connect(shadow._db_path()) as db:
        await db.execute(
            "INSERT INTO base_momentum_shadow_log (pool_address, chain, status, detected_at, entry_price, forward_pct_h2) "
        ...
        await db.execute(
            "INSERT INTO base_momentum_shadow_log (pool_address, chain, status, detected_at, entry_price) "
            "VALUES (?, ?, 'open', ?, 1.0)",
...
async def test_advance_exit_accumulates_window_volume_across_passages():
    ...
    window (same doctrine as solana_pump_shadow.py's twin test)."""
    await _insert_open_row(pool_address="poolA", entry_price=1.0, minutes_ago=10.0)
    detected_epoch = shadow._epoch_of((await _rows())[0]["detected_at"])
...
@pytest.mark.asyncio
async def test_run_cycle_also_advances_exit_simulation(monkeypatch):
    from aria_core.services.geckoterminal import TrendingPoolsResult
    ...
    assert result["exit_sim"]["checked"] == 0  # poolA has no price in FakeClient's map -> unavailable, skipped
...
def test_the_table_name_is_importable_as_a_constant():
    ...
    from aria_core import base_momentum_shadow as pocket

    assert pocket.TABLE == "base_momentum_shadow_log"
    assert "base_momentum_shadow_log" in pocket.DB_PATH or pocket.DB_PATH.endswith(".db")
...
async def test_every_refusal_is_recorded_so_the_filters_can_be_recalibrated():
    ...
    checked -- the exact defect class this module's own launch is meant to avoid
...
class TestRegimeGate:
    ...
    async def test_record_signals_blocks_a_new_entry_when_the_regime_is_shut(self, monkeypatch):
        ...
        regime gate is shut -- and the refusal must be tracked."""
        monkeypatch.setattr(shadow, "REGIME_MIN_MEDIAN_PEAK_PCT", 999.0)
        for i in range(shadow.REGIME_WINDOW):
            await shadow.record_regime_candidate(
    ...
    async def test_record_signals_opens_when_defillama_regime_is_absent_or_calm(self, monkeypatch):
        ...
        async def _no_reading(_chain):
            return None

        monkeypatch.setattr(shadow.chain_liquidity_regime, "latest_regime", _no_reading)

        n = await shadow.record_signals([_pool(pool_address="poolNoRegime", token_address="tokNoRegime")], chain=CHAIN)
        assert n == 1
