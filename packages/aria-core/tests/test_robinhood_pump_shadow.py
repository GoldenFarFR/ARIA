...
class FakeWsFeed:
    ...
    robinhood_pump_shadow.py -- _pools (membership check), add_pool/
...
async def test_evaluate_old_row_awaiting_h2_never_starves_a_younger_rows_m15():
    ...
    async with aiosqlite.connect(shadow._db_path()) as db:
        await db.execute(
            "UPDATE robinhood_pump_shadow_log SET forward_price_m15 = 1.1, forward_pct_m15 = 10.0, "
...
class TestRegimeGate:
    ...
    async def test_advance_regime_candidates_from_pools_a_pool_dropped_from_trending_still_expires(self):
        ...
        async with aiosqlite.connect(shadow._db_path()) as db:
            ...
            await db.commit()
        # a different pool in this cycle's response, poolGone nowhere in it
        stats = await shadow.advance_regime_candidates_from_pools(
...
class TestEnsureTableSelfHeals:
    ...
    async def test_ensure_table_recreates_after_external_drop_despite_stale_cache(self):
        async with aiosqlite.connect(shadow._db_path()) as db:
            await db.execute("DROP TABLE robinhood_pump_shadow_log")
            await db.commit()
