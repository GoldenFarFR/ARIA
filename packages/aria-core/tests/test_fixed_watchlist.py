"""02/08 -- "megacap" pocket's candidate source: a PERMANENT list of 10
...
@pytest.mark.asyncio
async def test_ensure_table_seeds_the_10_default_tokens():
    rows = await fw.list_watchlist_candidates()
    assert len(rows) == 10
    symbols = {r["symbol"] for r in rows}
    assert symbols == {
        "LINK", "KAITO", "ICP", "ENA", "AAVE", "VIRTUAL", "WETH", "CBBTC", "WBTC", "CBETH",
    }
