...
order is placed, pilot-scoped to scalping_v6/v7 (04/08). No real network
call, no real Telegram send: momentum_entry.fetch_candles/telegram_bot.
send_photo/chart_render are monkeypatched."""
...
@pytest.mark.asyncio
async def test_noop_outside_pilot_wallets(monkeypatch):
    """A swing/v1-v5/vc/megacap order must never trigger a fetch/send --
    pilot scope is scalping_v6/v7 only, an explicit operator choice."""
    called = []
    ...
    await limit_order_chart.maybe_send_order_chart(
        {"wallet": "swing", "contract": "0xabc", "chain": "base", "id": 1},
        {"pool_address": "0xpool"},
    )
    assert called == []
...
@pytest.mark.asyncio
async def test_pilot_wallet_with_pool_address_sends_photo_and_cleans_up(monkeypatch, tmp_path):
