...
from aria_core.x_publication_policy import check_tweet_allowed
...
def test_compose_workflow_used_tweet_no_url():
    ...
    assert "https://" not in tw
    assert "Virtuals ACP" in tw
    assert len(tw) <= 280
...
def test_enqueue_workflow_used(tmp_path, monkeypatch):
    ...
    assert "https://" not in row["tweet_text"]


def test_workflow_used_tweet_blocks_url(monkeypatch):
    ...
    allowed, reason, _ = check_tweet_allowed(tw, force=True)
    assert not allowed
