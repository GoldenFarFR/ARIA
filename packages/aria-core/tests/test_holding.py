...
async def test_holding_seed_and_structure(tmp_path, monkeypatch):
    ...
    assert structure.subsidiaries == []
    assert all(s.parent_id == structure.holding.id for s in structure.subsidiaries)


@pytest.mark.asyncio
async def test_retired_subsidiaries_purged_on_boot(tmp_path, monkeypatch):
    """Aria Market / DEXPulse were retired codenames — a prior boot may have seeded them;
    the next boot must clean them from the repertoire, not just stop re-seeding them."""
    db = tmp_path / "aria.db"
    monkeypatch.setattr("aria_core.repertoire_db.DB_PATH", str(db))
    await repertoire_db.init_repertoire_db()

    for slug, name in (("aria-market", "Aria Market"), ("dexpulse", "DEXPulse")):
        async with aiosqlite.connect(str(db)) as conn:
            ...
            await conn.execute(
                ...
                VALUES (?, ?, '', 'live', 'product', 0, 3, '', 0, '', ?, ?, 'subsidiary', NULL, ?)
                """,
                (f"legacy-{slug}", name, now, now, slug),
            )
            await conn.commit()
