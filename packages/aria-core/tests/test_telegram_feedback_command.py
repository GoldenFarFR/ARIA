...
@pytest.mark.asyncio
async def test_feedback_shows_all_active_pockets_distinctly(monkeypatch, tmp_path):
    ...
    single aggregate that silently hides the others. 18/08 -- v8/v9 retired,
    only swing+vc remain (see paper_trader.visible_reporting_wallets's
    docstring)."""
    from aria_core import paper_trader as pt
    ...
    monkeypatch.setattr(pt, "_run_cycle_lock", _asyncio.Lock())
    # real paper_state rows so all_reporting_wallets() enumerates both.
