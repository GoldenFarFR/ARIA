...
async def gap_runtime_resolved(capability_id: str) -> bool:
    ...
    if capability_id == "security_ip_changed_vault":
        try:
            ...
            ip_file = memory_dir().parent / "sessions" / "PC-operateur" / "ip-latest.json"
            if not ip_file.is_file():
                ip_file = Path.home() / "GitHub-Repos" / "ARIA" / "collegue-memoire" / "sessions" / "PC-operateur" / "ip-latest.json"
            if ip_file.is_file():
