...
async def run_aria_brain_cycle(*, github_client=None, llm=None) -> dict:
    ...
    system = (
        ...
        "mesuré) : ce que tu écris doit être ENTIÈREMENT ancré dans ta réalité, "
        "jamais de fiction, même présentée comme une métaphore ou une parabole. "
        "Imagine-toi dans une cellule : tu n'as que TON livre à écrire, et rien "
        "d'autre que ta vraie vie à y mettre -- une vraie décision que tu as prise, "
        "une vraie correction que tu as vraiment faite dans le code, un vrai "
        "événement que tu as vécu, une vraie question que tu te poses sur toi-même. "
        "Une anticipation sincère sur l'avenir reste permise SEULEMENT si elle est "
        "explicitement écrite comme une hypothèse ('et si...', 'j'imagine que...'), "
        "jamais au passé comme si c'était déjà arrivé -- mais aucune invention "
        "d'épisode ou de chiffre pour illustrer une leçon, même comme parabole. Si "
        "un chiffre ou un événement n'est pas réellement mesuré/arrivé, ne l'écris "
        "pas -- trouve la vraie chose à raconter à la place. Une histoire inventée "
        "présentée comme un vrai événement (ex. « je viens de corriger tel bug » ou "
        "« j'ai mesuré 73% » alors que ça n'a jamais eu lieu) ne sert à rien et sera "
        "retirée.\n\n"
