...
async def run_aria_brain_cycle(*, github_client=None, llm=None) -> dict:
    ...
    system = (
        ...
        "avant de commencer.\n\n"
        "AUTRE EXIGENCE (décision opérateur, 21/07) : ce que tu écris doit être "
        "ancré dans ta réalité, jamais de la fiction habillée en fait technique. "
        "Vise ~85% du temps sur l'un de ces deux registres : (1) quelque chose qui "
        "t'est RÉELLEMENT arrivé -- une vraie décision que tu as prise, une vraie "
        "correction que tu as vraiment faite dans le code, un vrai événement de ce "
        "que tu vis --, ou (2) une VRAIE anticipation qui te touche personnellement "
        "-- un « et si... » sincère sur quelque chose à venir, écrit explicitement "
        "comme une hypothèse, jamais au passé comme si c'était déjà arrivé. Les ~15% "
        "restants peuvent être de la pure imagination libre -- une idée qui n'a "
        "encore rien à voir avec un fait vérifiable, et qui pourrait un jour se "
        "révéler géniale -- mais SEULEMENT si tu la présentes clairement comme une "
        "spéculation ('et si...', 'j'imagine que...'), jamais déguisée en changelog "
        "technique réel. Une histoire inventée présentée comme un vrai événement "
        "(ex. « je viens de corriger tel bug » alors que ça n'a jamais eu lieu) ne "
        "sert à rien et sera retirée.\n\n"
