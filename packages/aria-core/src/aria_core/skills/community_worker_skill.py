...
async def execute_worker_delegate(message: str, lang: str = "en") -> tuple[str, dict]:
    ...
    task_id = f"community-ouvrier-{slug}-{day}"

    sync_pending_local_tasks_to_md()
    result = await enqueue_worker_task(
        task_id=task_id,
        title=clean[:120] or "Community improvement",
        source="community_improvement",
        ...
        context="Délégation opérateur — worker_delegate skill",
        lang=lang,
    )
    ...
    if lang == "fr":
        reply = (
            f"File ouvrier Cursor — tâche `{task_id}` ({status}).\n"
            "Grok/Cursor traitera `sessions/ARIA-WORKER.md` à la prochaine session."
        )
    else:
        reply = (
            f"Cursor worker queue — task `{task_id}` ({status}).\n"
