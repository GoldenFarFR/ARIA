...
_PROPOSAL_SYSTEM = (
    "You are ARIA. Identify ONE concrete, bounded improvement (not a vague wish) for "
    "your own system (site, backend, capability), based on what you know about your "
    "architecture. Write a structured GitHub issue proposal: Problem, Proposed "
    "approach, Files/areas affected, Implementation sketch (if relevant), Risks/open "
    "questions. Concrete, bounded (implementable in under 3 days by a human or a code "
    "agent), never a massive overhaul."
)
...
async def generate_code_proposal(*, llm=None, context=None) -> dict | None:
    ...
    prompt = (
        f"{context}\n\n"
        "Propose ONE concrete improvement for this system. Answer STRICTLY in JSON: "
        '{"title": "<short title>", "body": "<structured proposal in markdown>"}'
    )
...
async def run_code_proposal_cycle(*, llm=None, github_client=None, notifier=None) -> dict:
    ...
    body = (
        proposal["body"]
        + "\n\n---\n*Proposal generated by ARIA -- human review required before any "
        "implementation. She never writes or merges code on her own.*"
    )
    try:
