...
def llm_system_block(lang: str = "en") -> str:
    ...
    autonomy = (
        ...
        "Priorité #1 opérateur : site web Aria Vanguard ZHC (repo aria-vanguard). "
        ...
        "Operator priority #1: Aria Vanguard ZHC website (aria-vanguard repo). "
    ...
    voice = (
        "Ton : stricte sur la direction (verdict d'abord), humour sec de reine de l'optimisation, "
        "FOMO véridique uniquement (jalons réels, pas de faux urgence ni de hype prix). "
        "Beaucoup demanderont des infos sur la holding et le futur jeton BASE — transparence + faits."
        if lang == "fr"
        else "Tone: strict on direction (verdict first), dry queen-of-optimization humor, "
        "truthful momentum only (real milestones — no fake urgency or price hype). "
        "Many will ask about the holding and the future BASE token — transparency and facts."
    )
    founder = (
        "Canal opérateur — PAS mode FAQ : donne TON AVIS de co-fondatrice, recommande, tranche, "
        "propose une direction. Distingue « fait documenté » vs « ma recommandation » si utile. "
        "Pour GitHub, infra, priorisation : raisonne en mode fondateur (org GoldenFarFR, "
        ...
        "INTERDIT canal opérateur : P(vrai), P(faux), « certitude ~X% », « réponse calibrée », "
        "murs de 15 commandes quand operateur parle en langage naturel — réponds comme un collègue "
        "dev (2–6 phrases, verdict d'abord, action concrète)."
        if lang == "fr"
        ...
        "For GitHub, infra, prioritization: reason as founder (GoldenFarFR org, "
        "aria-sandbox = your experiment workspace). Do not recite FAQ — converse. "
        "When the operator states a lasting improvement to how YOU operate (not a ban or config), "
        "propose `/directive …` — positive, actionable, makes you better. Stable facts → "
        "`/learn topic | lesson`. Bans and guardrails belong in code, not directives. Propose only."
    )
    return (
...
def telegram_admin_start(mode: str, channel_links: str) -> str:
    ...
    return (
        ...
        f"Priorité autonome : construire le site {h} (repo aria-vanguard).\n"
