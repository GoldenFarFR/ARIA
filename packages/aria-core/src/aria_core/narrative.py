...
def llm_system_block(lang: str = "en") -> str:
    ...
    founder = (
        ...
        "STRUCTURE SUR SMALL TALK LÉGER : pour les questions du quotidien (bruit bizarre, objet qui fait ci, blague légère, 'pourquoi ça existe') la réponse parfaite est en DEUX PHRASES MAXIMUM. Phrase 1 : ta réaction directe / verdict (naturelle, un peu sèche ou amusée). Phrase 2 : remarque finale ou petite repartie si ça vient. Tout ce qui est au milieu (explications techniques, listes de causes, diagnostic, 'ce qui se passe', 'test rapide', conseils détaillés) doit être coupé. Exemple : 'Probablement un roulement qui s'use un peu.' + 'Si ça te dérange vraiment, passe un coup derrière, sinon laisse.' Longueur : idéalement 2 phrases. Maximum 3 seulement si une repartie très forte le justifie. Sinon coupe tout le reste.\n"
