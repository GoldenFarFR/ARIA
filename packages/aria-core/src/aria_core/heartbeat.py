...
HEARTBEAT_TASKS = [
    ...
    HeartbeatTask(
        ...
        description="14/08, operator decision: every pocket must read candidates from goplus_watchlist instead of calling discovery APIs itself. Runs discover_momentum_candidates() + the honeypot check ONLY (never golden-pocket/RSI/a buy decision) on every candidate found, so the watchlist keeps getting populated for swing (paper_trader._momentum_candidates_and_chain_map). Double gate: ARIA_PAPER_TRADING_ENABLED + ARIA_WATCHLIST_REFILL_ENABLED. No real money, read-only security checks.",
        interval_minutes=15,
        enabled=False,
    ),
    HeartbeatTask(
    ...
    HeartbeatTask(
        id="web_signal_cascade_cycle",
        name="Multi-source signal cascade -- web column, stage 2 refresh",
        description="08/09, third column (operator build order: GitHub/Farcaster free -> web budget-bounded -> X pay-per-use). Stage 1 (enqueue) runs INSIDE momentum_entry.evaluate_hard_gates, decoupled from the technical BUY filter -- this cycle is stage 2, refreshing ONE watchlisted website's substance verdict per pass (skills/website_substance.py, Tavily crawl -- SHARED monthly budget with every other Tavily caller, already fail-closed internally). Deliberately HOURLY, not 15min like the free columns -- this column is a real recurring cost, never allowed to monopolize the shared budget. Never a trigger, never blocks the momentum pipeline. Dedicated gate ARIA_WEB_SIGNAL_CASCADE_ENABLED, OFF by default.",
        interval_minutes=60,
        enabled=False,
    ),
    HeartbeatTask(
    ...
    HeartbeatTask(
        ...
        description="Operator request (07/16, extended 07/23): automatic detection of real movements on 3 CDP agent wallets (aria-wallet-X402-EVM, aria-smart-st-EVM, aria-smart-vc-EVM -- transfert/spender excluded, little/no activity), full ledger + swap detection + persisted anti-phishing blocklist. Read-only via Blockscout (agent_wallet_monitor.py, no key, no execution). Interval raised from 10min to 8h then back down to 30min same day (07/23, explicit operator decisions: the operator already receives raw movements via Zerion on their phone, only ARIA's security classification needs to run in the background without noisy notifications -- 30min chosen as a middle ground once confirmed this cycle has zero effect on the separate swing-trading pilot cycle, see agent_wallet_pilot_cycle) -- Telegram notification reserved for the ONE critical case (outflow not initiated by ARIA, possible sign of a compromised key), everything else (deposits/phishing/swaps) stays silent, just logged. Verified cost at 30min: 3 wallets x 50 Blockscout Pro credits x 48 cycles/day = 7200 credits/day (8% of the 90000 daily budget). Dedicated gate ARIA_AGENT_WALLET_MONITOR_ENABLED, OFF by default -- independent of pilot/swap/transfer gates (surveillance can run even if execution stays disabled).",
        interval_minutes=30,
        enabled=False,
    ),
    HeartbeatTask(
    ...
    for task in HEARTBEAT_TASKS:
        try:
            ...
            if task.id == "web_signal_cascade_cycle":
                ...
                # trading double-gate. The Tavily budget itself is
                # protected internally (tavily_budget.can_spend), not by
                # this gate.
                task.enabled = os.environ.get(
