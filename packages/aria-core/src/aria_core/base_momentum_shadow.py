    ...
    try:
        ...
        async with aiosqlite.connect(_db_path()) as db:
            for pool in pools:
                ...
                # how many concurrent open positions this pocket may hold, to
                # bound the live RPC/subscription cost per candidate.
                if await shadow_pocket_cap.at_capacity(
                    db, "base_momentum_shadow_log", open_clause="status = 'open'"
                ):
