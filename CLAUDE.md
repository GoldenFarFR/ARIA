...
- **3 réflexes inspirés de spec-kit (25/08)** : (1) clarification forcée — avant une recherche exploratoire multi-outils, énoncer l'objectif compris + les doutes avant de chercher, jamais enchaîner en silence ; (2) spec de chantier éphémère — pour un gros chantier multi-session, fichier de suivi dans `/opt/aria-data/task-specs/<sujet>.md` (hors git), supprimé à la clôture ou après 7j d'inactivité, tout ce qui est durable part vers sa destination officielle avant suppression ; (3) protocole bug-fixing assess → fix → test (diagnostic écrit avant correctif, correctif scopé, vérif contre le symptôme original, jamais juste "tests verts") — s'applique à mon travail sur le code, jamais à ARIA elle-même.
