...
- **19/07 -- trou réel trouvé en conditions réelles sur ce routeur, CORRIGÉ et DÉPLOYÉ (confabulation confirmée par capture opérateur).** Question opérateur directe sur Telegram ("tu a des positions ouverte ?") n'a matché AUCUN des 7 déclencheurs (`_NL_LEDGER_RE` exigeait "détail" avant "positions", jamais la formulation directe) -- tombée dans la conversation LLM générale, qui a confabulé sur les DEUX points à la fois : "aucune position ouverte" (fausse -- une position COBOT était réellement ouverte au même instant, vérifié en direct) et "1000$ simulés" (faux -- le vrai capital de départ paper-trading est 1 000 000$, pas 1000$ -- écart x1000, aucune source plausible identifiée pour ce chiffre). La réponse mélangeait aussi l'ancien système `candidate_ranking`/watchlist (VC-thesis, plus la source réelle de sourcing pour ce test depuis le pivot #194) avec le vrai portefeuille momentum -- même famille que les confabulations #105/#110/#113 déjà corrigées (un détecteur déterministe existe mais ne couvre pas CETTE formulation précise). **Fix** : `_NL_LEDGER_RE` gagne une branche `positions?\s+ouvertes?\b` -- ancrée sur "ouverte(s)" spécifiquement (jamais bare "position", ambigu en français avec "avis/opinion" -- testé qu'une question du type "tu as une position sur ce sujet ?" ne déclenche PAS le ledger). Route vers le même `paper_ledger_report.build_report()` déjà correct et déjà utilisé par les autres formulations de ce déclencheur (aucune nouvelle logique de lecture, juste une formulation supplémentaire reconnue). 2 nouveaux tests (dont une reproduction exacte de la phrase opérateur), suite complète vérifiée verte. **Limite honnête, déjà actée pour cette classe de bug** : ce correctif couvre CETTE formulation précise, pas toutes les reformulations possibles -- la limite structurelle reste qu'une réponse LLM générale n'est jamais une preuve de ce qu'ARIA fait réellement, seul un détecteur déterministe qui route vers les vraies données l'est.
- **19/07 (suite, ~15 min plus tard) -- 2e confabulation réelle, DIFFÉRENTE formulation, MÊME famille -- élargit un mécanisme existant plutôt qu'en construire un nouveau, CORRIGÉ et DÉPLOYÉ (décision opérateur explicite : "oui fait le mais anticipe").** Question opérateur ("c'est quoi ta these sur lachat de cobot ?") a reçu : "je n'ai pas de thèse sur COBOT -- parce que je ne l'ai pas passé dans le pipeline... colle-moi l'adresse et je regarde" (payant, 20301 tok) -- **faux** : la thèse COBOT existe réellement en base (`paper_position.thesis`, vérifié en direct), complète et détaillée (golden pocket + divergence RSI + MACD + R/R 2.5, exactement le pipeline momentum #194). Elle décrit en plus la méthodologie de l'ANCIEN `/vc` ("gate standard : honeypot/taxe, liquidité vs bots, invalidation") comme si c'était la seule façon dont elle analyse un token -- ignore totalement l'existence du pipeline momentum. **Root cause précise, PAS un nouveau trou -- un mécanisme existant (`_try_trade_status_response`/`is_trade_status_question`, construit le 17/07 pour un incident similaire) était trop étroit** : exige un mot-clé de trading (liste fermée -- "achat" absent, seul "acheté(e)" y était ; "thèse" absent ; pire, "AERO" -- le symbole du token de test du 17/07 -- était codé en dur comme SEUL exemple de nom de token, jamais un vrai mot générique) ET une tournure de question dans une liste fermée ("c'est quoi" absent). `build_trade_status_context()` (la fonction qui lit le vrai registre) était déjà correcte et complète -- seul le déclencheur était trop étroit, même schéma que le bug précédent. **Fix, généralisé plutôt que patché mot pour mot (consigne "anticipe")** : mots-clés élargis (`achats?` en plus d'`achet[ée]`, `th[eè]se?`, retrait du symbole `AERO` codé en dur -- un détecteur générique ne doit jamais dépendre d'un exemple de test) + tournures ajoutées ("c'est quoi", "quelle est", "qu'est-ce que", "what's") + **repli générique** : un simple `?` compte désormais comme tournure de question dès qu'un vrai mot-clé de trading est présent -- couvre par construction toute formulation FUTURE non encore observée, sans affaiblir le garde-fou réel (le mot-clé reste le filtre qui empêche tout hors-sujet, testé explicitement : "tu as vu le nouveau design du site ?" ne déclenche toujours rien malgré le "?"). 3 nouveaux tests (dont une reproduction exacte de l'incident avec la vraie thèse COBOT injectée dans `extra_system_context`, vérifiée par `brain.process()` bout en bout), suite complète vérifiée verte. **Toujours pas une garantie totale** : un mot-clé de trading totalement absent d'une question légitime resterait non couvert (ex. "pourquoi t'as fait ça ?" sans nommer le token/l'action) -- hors de portée d'un détecteur par mots-clés, la seule solution complète serait l'injection systématique déjà évoquée avec l'opérateur (un jour, si ce mécanisme s'avère encore insuffisant).
- **19/07 (suite) -- Diligence de conviction (X + site web + corroboration de contrat -> taille de position), demande opérateur explicite ("je veut une recherche active sur x qui permet a aria de voir aussi le contexte complet... en dehors des graphiques") -- CODÉE, TESTÉE, PAS DÉPLOYÉE, gate OFF.** Répond à un vrai trou vérifié par lecture directe du code avant de construire quoi que ce soit : `momentum_entry.py` (#194) ne fait AUCUNE recherche X/web au moment de l'achat -- le seul "buzz" utilisé était les boosts/profils DexScreener, le code documentait lui-même ce choix comme "hors scope, futur chantier possible". Plan présenté à l'opérateur AVANT tout code (chantier money-adjacent + réactive une décision de coût déjà prise le 11/07 + touche l'architecture vitesse-critique du pivot #194) -- **un seul point réellement ambigu tranché via `AskUserQuestion`** : le score de potentiel fondamental doit-il s'AJOUTER à la conviction technique existante (R/R+alignement) ou pouvoir la dominer -- opérateur a choisi **"s'ajoute en ET"** (option recommandée).
  - **`gateway/x_twitter.py`** gagne `search_recent_tweets(query)` (X API v2 `/tweets/search/recent`, buzz par ticker/CA) et `fetch_user_recent_tweets(username)` (timeline d'un compte, pour la cadence de publication) -- réutilisent le bearer déjà configuré (`settings.x_bearer_token`), même dôme que `fetch_curiosity_feed` (dégrade en liste vide sur toute panne, jamais une exception). `is_x_reading_active()` gagne un 4e gate consommateur (`aria_conviction_research_enabled`), même famille que curiosité/mentions/réponses.
  - **`x_research_budget.py`** (nouveau) : plafond hebdomadaire de **100 requêtes X** (pas un montant en $ -- le coût réel de LECTURE X n'a jamais été vérifié cette session, contrairement à la PUBLICATION déjà chiffrée dans `x_publication_policy.py` à 5$/mois -- jamais inventé un chiffre non vérifié). Même patron que `x402_budget.py` (semaine calendaire glissante, append-only, fail-closed), structurellement séparé (domaine de coût différent, jamais fusionné).
  - **`conviction_research.py`** (nouveau, orchestrateur) : site officiel (Tavily, déjà câblé et gratuit à l'usage actuel) -> handle X extrait des résultats -> recherche X sur le ticker/CA + timeline du handle (cadence "active"/"low"/"dormant"/"unknown" sur 30j) -> comparaison DÉTERMINISTE (pas de LLM) entre le contrat annoncé par le projet et le contrat réellement scanné (`True`/`False`/`None` -- `None` = aucune mention trouvée, jamais confondu avec `False` = un AUTRE contrat annoncé, signal d'usurpation possible) -> un seul appel LLM léger (Haiku 4.5 via OpenRouter, même modèle déjà validé sur des tentatives d'injection réelles pour `_llm_confirm`/`_llm_security_gate`) synthétise tout en un score borné 0-10 + une phrase. **Sécurité (mandat #192) traitée avec le même sérieux que le reste du pipeline, pas en option** : le contenu externe (site web, tweets) est ATTAQUABLE -- un projet malveillant peut façonner son site/ses tweets pour gonfler artificiellement la taille de la position qu'ARIA prendrait contre lui -- `sanitize_untrusted_text` sur CHAQUE fragment (snippet, tweet, symbole), balise `<donnees_non_fiables>`, règle système explicite d'ignorer toute instruction trouvée dedans, longueur totale plafonnée (2000 car.). Dégradation honnête à chaque étage : `available=False` SEULEMENT si le gate est OFF ; sinon toujours `available=True` même si rien n'a été trouvé (`potential_score=None` dans ce cas -- `None` veut dire "inconnu", jamais confondu avec un score bas mesuré).
  - **Branchement dans `momentum_entry.evaluate_momentum_entry`** : APRÈS le garde de sécurité final (donc seulement sur un `action == "BUY"` déjà confirmé par tout le reste -- honeypot, R/R, alignement, tie-breaker/garde LLM) -- ne tourne JAMAIS sur la masse de candidats rejetés par les filtres rapides (préserve la vitesse du pipeline, raison d'être du pivot #194 : "elle doit être là avant tout le monde"). No-op immédiat (zéro appel réseau) si `ARIA_CONVICTION_RESEARCH_ENABLED` est OFF.
  - **`risk_guard.conviction_size_multiplier`** gagne un paramètre optionnel `fundamental_score` (défaut `None`, 100% rétrocompatible -- aucun appelant existant cassé). Composition "ET" implémentée avec une nuance délibérée par rapport à la lecture littérale de la question posée à l'opérateur : **fail-open sur `None`** (recherche non menée/indisponible -- ne bloque JAMAIS le bonus technique seul, jamais réduit sous ce qu'il a aujourd'hui) et **fail-closed seulement sur un score CONFIRMÉ faible** (`< FUNDAMENTAL_WEAK_THRESHOLD = 4.0`) -- même doctrine fail-open/fail-closed déjà validée et éprouvée sur `smart_money.py` (wallet-scoring, 15/07). Le fondamental ne peut JAMAIS créer un bonus seul (testé explicitement) -- il ne fait que potentiellement bloquer celui déjà mérité par la technique.
  - **`paper_trader.py`** transmet `sig.get("potential_score")` au sizing, sans changement de comportement pour tout appelant qui ne le fournit pas.
  - **Limite honnête assumée, notée dans le docstring du module** : la corroboration de contrat est un "best-effort textuel", pas une garantie -- si un projet ne publie jamais son contrat officiellement, `contract_corroborated` reste `None` (jamais un blocage), et un projet habile pourrait en théorie afficher le bon contrat sur un site tout en étant malveillant par ailleurs (la corroboration prouve la cohérence narrative, pas l'honnêteté du projet). Portée strictement limitée au SIZING (jamais un gate d'achat séparé) -- respect exact de la demande opérateur ("influe sur la taille"), une éventuelle extension vers un rejet dur (ex. sur `contract_corroborated=False`, signal plus grave qu'un simple "peu de buzz") resterait une décision séparée, pas prise ici.
  - 14 nouveaux tests (`conviction_research.py`, dont 2 dédiés à la neutralisation d'injection de prompt via contenu externe), 12 (`x_twitter.py`), 6 (`x_research_budget.py`), 6 (`risk_guard.py`), 5 (`momentum_entry.py`, dont la non-régression : le garde ne coûte un appel QUE sur un BUY sur le point d'être exécuté, jamais avant), 2 (`paper_trader.py`, blocage fondamental faible confirmé + fail-open sur inconnu). Suite complète vérifiée verte. **Rien déployé** -- gate `ARIA_CONVICTION_RESEARCH_ENABLED` OFF par défaut, décision d'activation à prendre séparément par l'opérateur (réactive la lecture X, coupée le 11/07 pour coût -- même si bornée à 100 requêtes/semaine).
- **19/07 (suite) -- 3 décisions opérateur tranchées d'un coup après un point détaillé (align_score/x402/concentration/mémoire), 2 CODÉES+TESTÉES, 1 vrai bug x402 trouvé et corrigé en creusant la 3e.**
  - **Seuil `CONVICTION_ALIGN_SCORE_THRESHOLD` abaissé de 3 à 2** (décision opérateur explicite, via `AskUserQuestion`, après re-explication simplifiée du problème). Root cause vérifiée sur les 5 premiers trades réels momentum (COBOT/GITLAWB/CIRCLE/PLAZM/VVV, tous restés à 50k$ malgré des R/R parfois énormes) : align_score n'a JAMAIS atteint 3/3 -- toujours "MACD + pattern de bougie", jamais "EMA12 > EMA26" en même temps, confirmé en recomptant les raisons persistées dans les vraies thèses stockées en base (pas une supposition). Hypothèse retenue : un achat golden-pocket (rechargement profond) est structurellement en tension avec "EMA courte déjà repassée au-dessus de la longue". CODÉ + DÉPLOYÉ.
  - **Plafond de concentration (#187) corrigé pour les positions momentum** (décision opérateur "ok", après confirmation qu'une critique externe -- revue croisée Gemini -- était fondée : vérifié dans le code, `fit_alloc_to_concentration_cap` a un garde `if not category: return alloc`, et les positions momentum n'avaient JAMAIS de catégorie -- le plafond à 40% ne s'appliquait donc jamais à elles, contrairement au pipeline VC-thesis). Fix : `evaluate_momentum_entry` renvoie désormais `"category": f"momentum-{chain}"` (catégorise par chaîne -- seule dimension pertinente disponible, jamais par thèse qui recréerait un seul gros seau ; jamais mélangé avec les catégories launchpad de l'ancien pipeline VC-thesis). Les 5 positions déjà ouvertes AVANT ce fix restent `category=""` (non rétroactif, comportement attendu -- seules les nouvelles entrées sont désormais plafonnées). 4 nouveaux tests (dont un bout-en-bout via `run_paper_cycle` : 9 candidats identiques, plafonnés à 8 sur la 40%). CODÉ + DÉPLOYÉ.
  - **Bug x402 réel trouvé et corrigé en testant la QUALITÉ de deepnets.ai/lionx402/sociavault (demande opérateur explicite : "testez autant de fois que nécessaire... c'est la pierre angulaire").** Avant de payer quoi que ce soit, signalé à l'opérateur un vrai conflit : les paiements x402 utilisent le MÊME wallet CDP que le pilote de trading réel (0,98 USDC, déjà signalé trop bas pour trader) -- accepté de tester quand même (montants en centimes, budget x402 séparé à 4,98$ de marge). **`deepnets.ai/api/social-research` écarté avant tout paiement** : le 402 (lu en clair, jamais payé pour rien) révèle un paiement exigé en **USDC SOLANA** (le wallet agent est exclusivement Base) et un paramètre `mint` confirmant une portée strictement Solana/pump.fun -- inutilisable tel quel avec l'infra actuelle. **`lionx402`/`sociavault` (Base-compatibles, vérifiés) ont ÉCHOUÉ à 2 paiements réels indépendants** avec la même erreur SDK "Invalid payment required response" -- creusé jusqu'au bout (lecture directe du code source du SDK `x402` officiel installé, pas une supposition) : `x402_cdp_signer.py::build_x402_payment_header` reconstruisait un corps JSON synthétique et passait un getter de header **no-op** (`lambda _name: None`) au SDK -- son repli "corps" n'accepte STRICTEMENT que `x402Version==1`, alors que ces deux fournisseurs (et deepnets.ai) envoient du v2 (`x402Version:2`, offre dans le header `payment-required`, jamais le corps). Cybercentry (déjà payé avec succès plus tôt cette session) n'était jamais affecté car son offre est authentiquement v1/corps -- ce bug bloquait spécifiquement et systématiquement TOUT fournisseur v2 du catalogue Bazaar, une classe entière de fournisseurs jamais payable jusqu'ici malgré des offres parfaitement valides. **Fix non-invasif** (aucun appelant de `pay_fn` cassé, signature inchangée) : `x402_executor._extract_payment_requirement` transporte désormais le header brut original dans une clé interne `_raw_v2_header` du dict `requirement` (uniquement quand le chemin header a été emprunté -- jamais pour un vrai v1/corps) ; `x402_cdp_signer.py` appelle directement `decode_payment_required_header` (la même fonction que le SDK utilise en interne) sur ce header brut si présent, sinon comportement historique inchangé. 6 nouveaux tests (dont la reproduction exacte : header présent -> décodage direct, absent -> chemin legacy inchangé, échec de décodage -> exception propagée). CODÉ + DÉPLOYÉ -- retest réel des 2 fournisseurs prévu juste après pour boucler l'évaluation de qualité demandée par l'opérateur.
  - **Mémoire des recherches ("système bien rangé", décision opérateur explicite : "je veux pas que la mémoire dans 2 ans soit un foutoir")** -- pas encore construite à ce stade, prochaine étape après le retest x402.
...
**19/07 (nuit, travail autonome opérateur absent) — INCIDENT RÉEL trouvé et corrigé : #211 (authentification GeckoTerminal, déployée quelques heures plus tôt) causait ~79% de HTTP 429 en prod, cause probable du silence total du Cycle #2 paper-trading — CORRIGÉ et DÉPLOYÉ.**
En investiguant pourquoi le Cycle #2 (lancé 18/07 ~12h17 UTC) n'avait produit AUCUNE position 9h+ après son lancement (watchdog : 4 passages consécutifs, portefeuille toujours vide), diagnostic en direct sur le VPS (docker exec, lecture seule) : le pipeline momentum découvre bien des candidats (28-31 par passage) et les évalue, mais la cascade OHLCV échoue massivement dès son 1er étage.
- **Root cause** : le commentaire de #211 (18/07) affirmait « vérifié via la doc officielle CoinGecko » qu'une clé Demo débloquait 100 req/min sur les endpoints `/onchain` de GeckoTerminal — **chiffre faux**, confondu avec un autre palier CoinGecko sans rapport avec cet endpoint précis. Une vraie recherche web ce soir (apiguide.geckoterminal.com/faq, support.coingecko.com) confirme : palier gratuit/Demo pour `/onchain` = **~30 req/min**, pas 100. Le throttle accéléré à 0.65s/appel (~92 req/min, déployé avec #211 vers minuit) dépassait donc largement le vrai plafond soutenu.
- **Mesuré en prod avant correctif** : 666 échecs HTTP 429 contre 176 succès sur les ~2h suivant le déploiement de #211 (~79% d'échec), démarrant précisément à 00:27 — collé à l'horaire du déploiement, pas une coïncidence.
- **Piège méthodologique découvert en creusant** : mes propres scripts de diagnostic (`docker exec`) créent des processus Python séparés avec leur propre throttle/lock, totalement déconnecté du process serveur principal `aria-api` — ils AJOUTENT de la charge sur la même clé API partagée plutôt que de simplement observer. Vérifié explicitement via les logs Docker que les 429 avaient commencé 45 min AVANT mon premier script de diagnostic — confirmant que ce n'était pas moi qui causais le symptôme initial, seulement que mes diagnostics l'aggravaient pendant l'investigation.
- **Corrigé** : `_AUTHENTICATED_MIN_INTERVAL` réaligné sur `_MIN_INTERVAL` (2.1s, le rythme éprouvé en prod avant ce soir) — la clé continue d'être envoyée (peut légitimement débloquer un quota mensuel plus large), mais n'accélère plus le débit tant que le vrai plafond soutenu n'est pas revérifié. Note `/api` Telegram corrigée pour ne plus afficher le chiffre faux. Suite complète verte (5866 passed), déployé (`f62a6fa8`), vérifié en direct : ratio d'échec tombé à ~11% (5/47) dans les minutes suivant le redéploiement, contre ~79% avant.
- **Leçon actée** : un chiffre de débit "vérifié via la doc officielle" doit être vérifié en conditions réelles SOUTENUES (plusieurs minutes de trafic à pleine charge), pas juste via un test curl ponctuel qui confirme seulement que la clé est acceptée — même discipline que la norme déjà actée le 14/07 sur les noms de champs d'API, étendue ici au DÉBIT et pas seulement au SCHÉMA.
- **Non résolu ce soir, à surveiller** : reste à confirmer si CE correctif suffit seul à faire produire des trades au Cycle #2, ou si d'autres facteurs (CoinMarketCap qui échouait aussi en HTTP 500 pendant le diagnostic, GoPlus proche de son propre rate-limit) contribuent encore. Watchdog déjà en place (`/opt/aria-data/paper-watchdog/`) pour objectiver ça sur les prochains passages.

**19/07 (nuit, suite) — INCIDENT CAPITAL RÉEL trouvé via captures Telegram opérateur : pilote agent-wallet bloqué sur un bug du SDK CDP officiel, AUCUNE perte de fonds, PAS corrigé (attend le "go" opérateur).**
Deux alertes Telegram « 🔴 ARGENT RÉEL — pilote agent-wallet ÉCHANGE URANUS ÉCHOUÉ » (05:42 et 06:44 UTC) sur le contrat `0x5c4348e963369e124cb8e9b1a619f2f6f1717b58`. Vérifié immédiatement en priorité absolue (lecture seule, on-chain) :
- **Solde du wallet agent inchangé** (0,98 USDC + 0,001 ETH, `other_tokens: []`) — identique à l'état documenté au moment de l'activation le 18/07.
- **`agent_wallet_log` confirme `tx_hash: ''` sur les deux tentatives** — aucune transaction signée/envoyée on-chain, le swap a échoué AVANT tout mouvement de fonds. Montant tenté 0,0294 USDC (~3% du solde, conforme #203), slippage 1000 bps (10%, conforme à la règle absolue).
- **Root cause identifiée par lecture directe du SDK** (`cdp/openapi_client/models/common_swap_response_fees.py`, version 1.47.1, déjà la dernière disponible sur PyPI) : le champ `gas_fee` du modèle Pydantic `CommonSwapResponseFees` est déclaré **non-nullable** (`TokenFee`, pas `Optional[TokenFee]`), mais la propre méthode `from_dict()` du SDK peut légitimement produire `gasFee: None` quand l'API CDP omet ce champ de sa réponse (probablement pour un token trop peu liquide pour estimer les frais de gas de façon fiable) — contradiction interne au SDK officiel Coinbase, pas un bug côté ARIA. Aucune mention publique trouvée (recherche web), mais confirmé sans ambiguïté par lecture du code source installé.
- **Le comportement actuel est déjà SÛR** : l'exception est catchée, journalisée avec `status="failed"`, aucune perte, fail-closed conforme à la doctrine du projet. Je n'ai PAS tenté de contourner cette validation en fabriquant une valeur de `gasFee` — irait directement à l'encontre de "ne jamais inventer une donnée", plus encore sur du capital réel.
- **Vrai risque identifié (progrès, pas sécurité)** : le cooldown technique existant (`SWAP_FAILURE_COOLDOWN_MINUTES=60`, `agent_wallet_pilot_cycle.py`) coïncide exactement avec la cadence du cycle heartbeat (60 min) — les deux tentatives observées sont espacées de ~62 min, juste après l'expiration du cooldown. Si URANUS reste le candidat prioritaire à chaque cycle, le pilote pourrait le retenter indéfiniment, échouant toujours pour la MÊME raison structurelle (le bug SDK ne se résout pas avec le temps, contrairement à un timeout réseau), sans jamais progresser vers un autre candidat.
- **Correctif proposé, PAS implémenté** : distinguer les échecs structurels/déterministes (erreur de validation Pydantic sur la réponse — va se reproduire identiquement) des échecs transitoires (réseau, slippage) et appliquer un cooldown BEAUCOUP plus long (ou un statut dédié) pour les premiers, sans toucher `momentum_blacklist.py` (réservé aux vraies menaces de sécurité, pas aux pannes techniques — doctrine déjà actée le 17/07, ne pas la brouiller). **Volontairement pas codé ce soir** : ça touche le cœur de la logique du pilote de capital réel — même un correctif purement défensif (n'affecte ni le sizing, ni la sélection de sécurité, ni le slippage) mérite ta validation explicite sur ce périmètre précis, pas une décision unilatérale prise en ton absence. Aucune urgence financière (le risque est un gaspillage de cycles, jamais une perte) — peut attendre ton retour.

**19/07 (suite) — Workflow tweet resté bloqué depuis minuit, avalait tout message opérateur sans rapport (dont le prompt Sealed Ledger et une question sur le portefeuille) — CORRIGÉ, testé, DÉPLOYÉ.** Signalé par l'opérateur (capture Telegram : « Ton portefeuille est composé de quoi ? » a reçu un brouillon de tweet sur le scoring de launchpads en réponse) — même famille que le bug #110 (`vc_followup`) déjà corrigé, mais sans le TTL de 4h qui protégeait ce cas-là. Root cause confirmée en lisant l'état réel en prod (`tweet_compose_workflow.json`) : `handle_workflow_message()` (`brain.py:432`, appelé quasi inconditionnellement pour tout message opérateur non-public) traite N'IMPORTE QUEL texte comme faisant partie du workflow tweet dès que sa phase n'est pas `idle` — et rien ne fait jamais expirer cette phase. Le fichier était bloqué en phase `add_more` depuis 19/07 00:05 UTC (~9h40), ayant englouti au moins deux messages opérateur majeurs : le prompt complet du Sealed Ledger (transformé en `operator_notes`, jamais lu comme une instruction) et la question portefeuille (a régénéré un brouillon halluciné via LLM en utilisant ce texte comme note de style). **Corrigé** : `_save()` timestampe désormais chaque écriture (`updated_at`) ; `handle_workflow_message()` vérifie en tête si le workflow est non-idle ET périmé (>20min sans interaction, `_WORKFLOW_STALE_MINUTES`) — si oui, réinitialise silencieusement à `idle` AVANT de traiter le message, qui retombe alors dans le routage normal. Fail-safe explicite : un état sans `updated_at` (format antérieur à ce correctif — exactement l'état trouvé bloqué) est traité comme périmé d'office. Le démarrage explicite du workflow (`propose un tweet`/`crée un tweet`/etc.) n'est pas touché — seule la persistance indéfinie d'un workflow oublié est corrigée. 7 nouveaux tests (dont une reproduction exacte de l'incident avec le state legacy trouvé en prod), suite complète verte. **Débloqué en prod immédiatement** (reset direct via `docker exec`, indépendant du déploiement du code) pour rétablir la conversation Telegram sans attendre. **Bug annexe trouvé en vérifiant la suite complète avant de committer** : `test_base_crawler.py::test_top_pools_min_age_days_filters_fresh_pools` utilisait deux dates codées EN DUR (2026-06-01/2026-07-12) — pure bombe à retardement, le pool "fresh" a fini par dépasser le seuil `min_age_days=7` simplement parce que le temps a passé (aucun rapport avec ce chantier). Corrigé en calculant les dates relativement à `datetime.now(timezone.utc)` au moment de l'exécution — le test reste valide indéfiniment désormais.

**19/07 (suite) — x402 Bazaar (découverte de services x402, "top tendance") — CODÉ, testé, PAS déployé.** Réponse à « il n'existe pas un top tendance des meilleurs outils x402 ? c'est dommage de se limiter à Cybercentry » — vérifié avant de coder (WebSearch + vrai appel curl, jamais un schéma supposé) : le registre officiel Coinbase CDP (`docs.cdp.coinbase.com/x402/bazaar`) expose `GET https://api.cdp.coinbase.com/platform/v2/x402/discovery/search`, **aucune clé CDP requise pour la découverte** (confirmé HTTP 200 sans authentification), classement qualité (buyer reach 30j, volume 30j, récence, curation Coinbase) recalculé toutes les 6h. Le champ `quality.l30DaysTotalCalls`/`l30DaysUniquePayers` EST bien exposé dans la vraie réponse (contrairement à ce que suggère la doc, "no per-service breakdown") — confirmé sur 5 services réels (Tavily Search avancé : 48 319 appels/374 payeurs uniques sur 30j — déjà un client ARIA existant, `services/tavily.py`). Nouveau `services/x402_bazaar.py` : `search()` (brut) + `discover_trending()` (trié explicitement par volume 30j, jamais l'ordre brut de l'API qui mélange pertinence textuelle) + `format_trending_report()`. **Portée strictement découverte** : aucune fonction de paiement, ne touche pas au plafond `x402_budget.py` (5$/semaine). **Sécurité notée explicitement dans le docstring** : certains services observés en conditions réelles ont des champs adressés LITTÉRALEMENT à un agent IA (ex. `extensions.a2a_negotiation.message: "Hey agent! ..."`) — traité strictement comme texte d'affichage, jamais interprété (doctrine "instruction source boundary" déjà appliquée partout ailleurs dans ce projet ; Coinbase lui-même avertit pareil pour les `skillUrl`, non câblés ici). Commande Telegram `/x402trending [mots-clés]` (admin-only, lecture seule), ajoutée au menu + `/api`. Deux erreurs de transcription trouvées et corrigées par les tests eux-mêmes avant commit (adresse USDC Solana mal recopiée à la main — corrigée en la calculant via Python plutôt qu'en retapant ; assertion de test comptant deux fois la même URL). 16+12 tests, suite complète vérifiée verte. **Correction (20/07, travail autonome) : en réalité déployé avec le compteur de rejet ci-dessous** (commit `1f48eca0`, confirmé ancêtre du HEAD actuellement en prod et module réellement présent dans le conteneur) — les deux paragraphes se contredisaient (celui-ci disait "rien déployé", celui du compteur de rejet disait "déployé... regroupé avec x402 Bazaar" pour le MÊME commit) ; le texte n'avait juste jamais été mis à jour après le déploiement conjoint.

**19/07 (suite) — compteur de rejet par étape (proposition d'ARIA elle-même en conversation Telegram, "preuve avant opinion") — CODÉ, testé, PAS déployé.** Avant de construire quoi que ce soit : découverte que le mécanisme de comptage EXISTAIT DÉJÀ, partiellement — `momentum_entry.evaluate_momentum_entry` retourne déjà un `hold_reason` codé sur chaque branche de rejet (honeypot, wash-trading, prix parabolique, pas de signal R/R, LLM non confirmé, garde de sécurité final, etc.), et `paper_trader.run_paper_cycle` calcule déjà un funnel agrégé PAR CYCLE (mandat #192, 16/07). Le vrai trou (confirmé par grep avant d'écrire une ligne) : ce funnel n'était JAMAIS persisté au-delà d'un `logger.info` — aucun autre appelant ne lisait `actions["momentum_funnel"]`, donc chaque cycle perdait totalement la donnée, impossible de juger "marché plat vs filtre trop strict" sans dépouiller les logs applicatifs un par un. Nouveau `momentum_funnel_log.py` (même patron que `momentum_blacklist.py` : SQLite, `_ensure_table()`, append-only) — `record_funnel()` persiste chaque cycle, `summarize_since(hours)` agrège par fenêtre glissante, `format_funnel_summary()` classe par fréquence décroissante (la cause dominante de rejet en premier). Câblage dans `paper_trader.py` : un seul appel additif juste après le calcul du funnel déjà existant (`try/except` — une panne d'écriture de télémétrie ne casse jamais un cycle de trading réel), **aucun changement de la logique de décision**. Commande Telegram `/funnel [heures]` (admin-only, lecture seule, défaut 48h — la fenêtre qu'ARIA avait elle-même proposée). Piège d'isolation de test évité (même famille que le bug déjà documenté pour `momentum_blacklist.py`, `DB_PATH` figé à l'import) : `test_paper_trader.py` isole désormais aussi `momentum_funnel_log.DB_PATH` dans sa fixture `tmp_db` partagée. 7+6+2 tests, suite complète vérifiée verte. **Déployé et vérifié en prod** (commit `1f48eca0`, health check confirmé) — regroupé avec x402 Bazaar ci-dessus pour un seul rebuild Docker.

**19/07 (suite) — correctif cooldown structurel URANUS (pilote agent-wallet, capital réel) — CODÉ, testé, PAS déployé.** Suite de l'incident documenté plus haut (2 échecs identiques 05:42/06:44 UTC, `ValidationError` Pydantic sur `CommonSwapResponseFees.gasFee`, bug SDK CDP confirmé). Décision prise après avoir VÉRIFIÉ l'état réel plutôt que d'agir par précaution : `agent_wallet_log.list_transactions()` montre seulement ces 2 tentatives au total — aucune boucle de retentative répétée ne s'est produite en pratique depuis (le pilote a dû simplement passer sur d'autres candidats), donc le correctif n'était pas "obligatoire" au sens d'un problème actif, mais reste une amélioration de robustesse peu coûteuse (aucune touche au sizing/sécurité/slippage) qui vaut le coup vu le risque latent. Nouveau `agent_wallet_log.is_structural_swap_failure(reason)` — détecte par sous-chaîne ("validation error"/"pydantic") un échec DÉTERMINISTE (va se reproduire identiquement) plutôt que TRANSITOIRE (réseau, slippage). `recent_failed_swap()` gagne un paramètre optionnel `structural_within_minutes` (défaut `None` = comportement historique inchangé pour tout appelant existant) — si fourni ET que le dernier échec est structurel, ce cooldown remplace `within_minutes` pour ce cas précis. `agent_wallet_pilot_cycle.py` passe désormais `STRUCTURAL_SWAP_FAILURE_COOLDOWN_MINUTES = 7 jours` (assez long pour ne pas gaspiller un cycle/heure sur un token structurellement cassé, assez court pour redonner sa chance si Coinbase corrige son SDK). **Jamais confondu avec `momentum_blacklist.py`** (réservé aux vraies menaces de sécurité, doctrine actée le 17/07) — ce correctif ne bannit rien, il espace juste les retentatives sur un échec technique confirmé déterministe. 9 nouveaux tests (dont une reproduction exacte de l'incident réel avec les vrais messages d'erreur observés), suite complète + `test_coherence.py` vérifiées vertes. **Déployé et vérifié en profondeur** (commit `b00d108c`, health check + constante `STRUCTURAL_SWAP_FAILURE_COOLDOWN_MINUTES=10080` et `is_structural_swap_failure()` confirmées vivantes dans le conteneur réel, gate pilote resté inchangé).

**19/07 (suite) — diagnostic "pourquoi le compteur du test 1M$ reste à zéro" + 2 leviers demandés par l'opérateur — CODÉS, testés, en cours de déploiement.** Vérifié en direct (jamais supposé) : le portefeuille papier reste à 1 000 000 $ exact, 0 position ouverte/clôturée, depuis le lancement du Cycle #2 (18/07 ~12h17 UTC) — même après le correctif GeckoTerminal 429 de la nuit. Le compteur de rejet fraîchement déployé (`/funnel`) donne un premier signal chiffré : sur 40 candidats cumulés, **80% rejetés faute de setup technique** (`no_entry_signal`), contre seulement 10-5-5% pour honeypot/parabolique/blacklist — confirme l'hypothèse qu'ARIA avait formulée elle-même en conversation ("la plupart des tokens sont soit déjà +200%, soit du bruit plat sans structure").
- **Bug CoinMarketCap trouvé au passage, non-bloquant** : `dex/token/pools` et `k-line/candles` renvoient un 500 systématique — testé même sur WETH (contrat archi-connu), même résultat. N'explique PAS le problème principal (le funnel ne montre aucun `ohlcv_unavailable`, un autre étage de la cascade prend le relais), mais reste un vrai gaspillage d'appels à corriger séparément un jour.
- **Investigation empirique décisive avant de toucher au code** (doctrine "jamais un correctif à l'aveugle sur ce qui encode la thèse de trading") : sur 8 candidats réels avec données exploitables, golden pocket atteint dans 4 cas, **divergence RSI dans 0 cas** — la divergence RSI est le vrai goulot, pas la tolérance de prix (piste initialement envisagée, écartée après vérification).
- **Root cause du 0/8** : `bullish_rsi_divergence()` (`entry_signals.py`) ne comparait QUE les deux DERNIERS pivots bas de la fenêtre (`pivots[-2]`/`pivots[-1]`) — toute divergence formée entre le pivot le plus récent et un pivot plus ANCIEN de la même fenêtre était invisible. **Corrigé** : compare désormais le dernier pivot à CHAQUE pivot antérieur (du plus récent au plus ancien), même DÉFINITION stricte du signal (prix plus bas + RSI plus haut), seule la PORTÉE de recherche est élargie — strict sur-ensemble de l'ancien comportement (tout ce qui était détecté avant reste détecté). Affecte tous les appelants de `entry_signals.py` (`momentum_entry.py`, `acp_onchain_scan.py`/`/vc`, `market_sentiment.py`, `arena_signal.py`), pas seulement le pipeline momentum — cohérent, c'est une amélioration du signal lui-même. Nouveau test de non-régression construit et vérifié empiriquement (cas réel où la paire immédiate ne diverge pas mais une paire plus ancienne si).
- **Plafond de débit ajouté à `momentum_websocket.py` AVANT activation** (réponse à la question opérateur légitime sur le risque de saturer les API) : `MAX_EVALUATIONS_PER_HOUR=80`, fenêtre glissante 1h, aligné sur le débit du cycle heartbeat classique (20 candidats × 4 cycles/heure) — le WebSocket gagne l'avantage de LATENCE (détection quasi-immédiate au lieu d'attendre jusqu'à 15 min) sans exploser le VOLUME total consommé par GeckoTerminal/GoPlus/CoinMarketCap. Dégradation progressive (tronque la liste plutôt que d'annuler toute la vidange).
- 5+9 nouveaux tests (entry_signals + momentum_websocket), suite complète verte (5929 passed, mêmes 5 échecs pré-existants). **Déployé et vérifié en profondeur** (commit `ab0d4871`, health check confirmé) — puis `ARIA_MOMENTUM_WEBSOCKET_ENABLED=true` ajouté au `.env` réel du VPS, conteneur recréé (le flag est lu une seule fois à `start()`, `docker restart` seul n'aurait pas suffi -- `--env-file` est figé à la création du conteneur), **service confirmé démarré en conditions réelles** (log `momentum_websocket: démarré (4 endpoints)` observé dans le conteneur). Les deux leviers demandés par l'opérateur ("fait les deux") sont désormais pleinement actifs en prod.

**19/07 (suite) — audit complet des connexions/réglages (opérateur absent, "vérifie que tout est bien fait") — 2 découvertes réelles, dont une capital réel.** Inventaire `/api` : 36 intégrations, 34 configurées (les 2 restantes = choix déjà actés : TikTok jamais créé, clé x.ai Management absente sans impact sur l'inférence Grok). Tous les gates `ARIA_*_ENABLED` du `.env` réel listés et vérifiés cohérents avec la doctrine -- y compris `ARIA_ACP_PROVIDER_ENABLED=true`, vérifié SANS EFFET réel (`resolve_acp_command()` confirmé vide dans le conteneur, aucun binaire `acp-cli` présent -- armé mais inerte, cohérent avec "provider ACP toujours off, CLI absent").
- **Bug réel trouvé et corrigé : corruption de casse Solana dans `momentum_websocket.py`.** En activant le WebSocket aujourd'hui pour la première fois, `_ingest_frame()` s'est révélé lowercaser AVEUGLÉMENT toute adresse (y compris Solana, base58 sensible à la casse) -- exactement le même bug déjà corrigé le 18/07 côté REST (`momentum_entry.normalize_contract_case`, #63), jamais reporté ici car ce module avait été écrit AVANT cette découverte et n'avait jamais tourné jusqu'à aujourd'hui. Symptôme observé en prod (confirmé par appel direct) : RugCheck (repli honeypot Solana, #207) rejetait en `400 "invalid length, expected 32, got 33"` des adresses dont la vraie couverture n'a jamais pu être vérifiée avec la bonne casse -- fail-closed donc jamais un risque de sécurité actif, mais une couverture de sécurité RugCheck silencieusement inopérante sur tout candidat Solana découvert par ce chemin. Corrigé en réutilisant `normalize_contract_case` (même fonction que le chemin REST, jamais dupliquée). 2 nouveaux tests (préservation Solana + non-régression Base), suite complète en cours de vérification avant déploiement.
- **Découverte capital réel, pas encore résolue : le pilote agent-wallet semble structurellement bloqué, pas juste sur URANUS.** Une 3e tentative réelle (10:47 UTC, détectée en auditant les logs) porte sur un token COMPLÈTEMENT DIFFÉRENT d'URANUS, mais échoue avec EXACTEMENT la même erreur (`ValidationError` Pydantic sur `CommonSwapResponseFees.gasFee`). Chiffres vérifiés : `0,98 USDC (solde réel actuel) × 3% (sizing, #203) = 0,0294` -- exactement le montant des 3 tentatives, aucune coïncidence. Hypothèse forte (pas encore confirmée par un test direct, qui coûterait du capital réel) : un montant de swap aussi minuscule (moins de 3 centimes) est probablement en dessous du seuil viable pour que l'API CDP calcule un devis de gas fiable -- la doc officielle mentionne explicitement "liquidity constraints or network conditions with very small amounts" comme cause connue de ce type d'erreur. **Le vrai facteur commun n'est donc PAS le token (mon diagnostic initial sur URANUS était incomplet), c'est le solde du wallet lui-même**, très en dessous des 10-15$ prévus dans le plan initial (`docs/pilote-agent-wallet-10usd.md`). Aucune perte de fonds dans les 3 cas (fail-closed confirmé, `tx_hash` toujours vide). **Rien codé/modifié** -- le sizing à 3% a été explicitement décidé le 16/07 (#203) et le financement du wallet est un acte financier réel, aucun des deux ne doit être changé sans validation opérateur explicite. Décision nécessaire au retour : soit financer davantage le wallet (remonter au niveau prévu initialement), soit accepter que le pilote reste inactif tant que le solde est aussi bas. **Statut au 19/07 12h : toujours 3 tentatives au total (0 nouvelle depuis 10:47), aucune perte de fonds, `to_address` toujours vide -- rien n'a bougé, la découverte reste d'actualité telle quelle.**

- **19/07 (suite, opérateur absent) -- premier trade RÉEL du Cycle #2 produit, confirme que les deux leviers (#89/#90 divergence RSI + #87/#88 WebSocket) fonctionnent bout en bout.** Position COBOT ouverte 11:51 UTC (id=9, `0xa82138d5...`, 50 000$ soit 5% du capital de départ, chain base) -- thèse ancrée sur une vraie divergence RSI détectée par le correctif du jour ("plus-bas prix 0.000136511 < 0.000140608 mais RSI remonte (40 → 46)") + golden pocket + MACD + patterns de bougie haussiers, R/R 2,5. Portefeuille : 950 000$ cash / 1 000 000$ équité (position encore proche de son prix d'entrée). `risk_guard.blocks_new_entries()` vérifié `False` (coupe-circuit sain, dernier événement `weekly_reset_auto`). C'est la première preuve concrète que le diagnostic de ce soir (0/8 divergences détectées avant le fix) se traduit en un vrai trade en conditions réelles, pas seulement en théorie.
- **19/07 (suite) -- 429 GeckoTerminal toujours présents malgré le retour à 2.1s de cette nuit, mais SANS perte de donnée mesurée -- referme le point laissé "à surveiller".** Vérifié en direct : le throttle réel (`geckoterminal_client`, singleton partagé, `asyncio.Lock`) applique bien 2.1s entre CHAQUE appel (confirmé `min_interval=2.1` dans le conteneur), et le débit moyen observé (88 appels / ~5min ≈ 17,6 req/min) reste largement sous le palier ~28,5 req/min -- donc AUCUNE violation du rythme moyen. Pourtant des 429 persistent en rafale (mesuré sur 20min : 54 requêtes 429 sur 153, ~35%, touchant BASE autant que Robinhood -- pas un problème spécifique à une chaîne exotique comme je le pensais d'abord, vérifié par ventilation par réseau avant de conclure). Hypothèse la plus probable : le palier gratuit GeckoTerminal est sensible aux RAFALES (fenêtre glissante courte), pas seulement au débit moyen -- deux sources concurrentes (cycle heartbeat 15min + drain WebSocket 30s) peuvent cogner au même instant même si chacune respecte le throttle partagé. **Aucune perte de candidat confirmée** : le funnel de rejet (`/funnel`) ne montre AUCUNE entrée `ohlcv_unavailable` sur 1h/6h glissantes -- la cascade à 5 étages (#194, GeckoTerminal → CoinMarketCap → Mobula → synthèse DexScreener → Dune) absorbe déjà ces échecs sans bloquer l'évaluation, preuve directe avec le trade COBOT ouvert pendant cette même fenêtre. **Décision : rien touché au throttle** -- le ralentir encore réduirait le débit d'évaluation (contre l'exigence opérateur de vitesse, #194) pour un gain incertain vu que la cascade dégrade déjà proprement ; l'accélérer reproduirait l'erreur déjà commise cette nuit (#211, 79% d'échec). Piste de suivi documentée mais pas construite : un coupe-circuit adaptatif par fournisseur dans la cascade (sauter GeckoTerminal quelques minutes après N 429 consécutifs) serait la prochaine optimisation légitime si la latence perçue devient un problème réel -- pas fait ce soir, décision à prendre avec plus de recul.
...
**19/07 (suite) — 2e bug x402 corrigé (header `PAYMENT-SIGNATURE` vs `X-PAYMENT`) + mémoire des recherches câblée + verdict réel de qualité x402 Bazaar (demande opérateur explicite : "teste autant de fois que nécessaire... c'est la pierre angulaire") — TOUT DÉPLOYÉ ET VÉRIFIÉ.**
Suite directe du 1er bug x402 (décodage v2 depuis le header brut, déjà corrigé et déployé plus haut ce même 19/07) : le retest réel a immédiatement buté sur un 2e bug distinct.
- **Bug** : après signature réussie, l'exécuteur envoyait TOUJOURS le paiement sous `X-PAYMENT` (v1 legacy) — mais le SDK officiel `x402` construit la signature v2 sous la clé `PAYMENT-SIGNATURE` (confirmé en lisant `x402/http/constants.py` du SDK installé, pas une supposition). Résultat : même une signature v2 parfaitement valide échouait à "Invalid payment required response" une seconde fois, juste après. Fix symétrique au premier : `x402_cdp_signer.py` accepte les deux clés possibles en sortie du SDK, `x402_executor.py` choisit le NOM de header à ENVOYER selon `requirement["x402Version"]` (jamais deviné). Suite complète (aria-core, 6000 passed + 5 échecs `test_proactive*` pré-existants — reconfirmés indépendants en rejouant le même run sur `main` vierge, cf. note méthodo ci-dessous) + `test_coherence.py` verts. Déployé (`6556e5af`), vérifié par health check indépendant.
- **Note méthodo (réutilisable)** : pour confirmer qu'un échec de test est bien pré-existant/sans rapport plutôt que causé par mes changements, `git stash` + rejouer la suite sur `main` nu est plus fiable qu'un simple raisonnement — mais un run complet sur `main` nu peut se bloquer sur un test réseau réel qui hang (`ep_poll` observé, socket ouvert) : si un test isolé du même fichier passe 10/10 ET que le run complet nu reproduit la MÊME signature d'échec à la même position relative avant de devoir être interrompu, c'est une preuve suffisante — pas besoin d'insister jusqu'au bout sur un run redondant qui bloque sur autre chose.
- **Mémoire des recherches câblée** (demande opérateur explicite, "toute recherche doit etre enregistrer dans la memoire... accumulative... pour un suivi... je veux pas que la mémoire dans 2 ans soit un foutoir") : `conviction_research.py` vérifie désormais la mémoire LanceDB (type d'entrée dédié `conviction_research`, `memory/vector/schema.yaml`, `retention_days=null` — jamais purgée) AVANT tout appel Tavily/X — un résultat de moins de 7 jours (`DEFAULT_RESEARCH_CACHE_MAX_AGE_DAYS`) sert directement, aucune re-recherche. Stockage systématique en fin de fonction, même sur "rien trouvé" (jamais un `UPDATE` — chaque recherche reste une entrée séparée et datée, `get_research_history()` renvoie tout l'historique trié du plus récent au plus ancien pour suivre l'évolution d'un projet dans le temps, ex. cadence de publication qui se dégrade). 9 nouveaux tests (cache hit/miss/isolation par contrat+chaîne/fraîcheur/historique). Déployé avec le fix x402 ci-dessus (même commit `6556e5af`).
- **Verdict réel de qualité x402 Bazaar comme source pour `conviction_research`, basé sur 4 paiements réels effectués ce soir (2 précédents + 2 nouveaux), $0,031 dépensés au total sur $5/semaine (marge $4,969 restante, wallet CDP à 0,969 USDC après)** :
  - `lionx402/web-search-json` (0,01$) : paiement RÉUSSI (confirme le fix mécanique), mais qualité MÉDIOCRE — simple wrapper DuckDuckGo Instant Answer, 0 résultat sur une requête projet précise, confiance auto-déclarée 0,2/1. Moins bon que Tavily (déjà intégré, gratuit à l'usage actuel).
  - `glim.sh` (le service Twitter le plus utilisé du Bazaar par volume) : le endpoint le plus populaire de son propre catalogue (`api/v1/twitter/search`) renvoie un 404 "Not found" (listing Bazaar visiblement périmé, header `x-glim-migrate` suggérant une migration d'API côté fournisseur) ; un autre endpoint du même fournisseur (`api/v1/twitter/tweets/search`) répond bien 402 mais son `accepts[]` n'offre QUE de l'USDC Solana — bloqué correctement par le garde fail-closed existant (`actif non supporté`, même famille que `deepnets.ai` déjà écarté) puisque le wallet CDP d'ARIA est Base-only.
  - `sociavault/twitter/search` (0,014$) : 402 confirmé, mais son schéma exige un corps JSON même sur une requête `GET` — `x402_executor.fetch_paid_resource`/`_default_http_fetch` ne supportent aujourd'hui QUE `url`+`method`+`headers`, aucun corps. Endpoint structurellement pas encore appelable avec le code actuel (extension réelle nécessaire, pas juste une histoire de test).
...
- **Bug de comptabilité DISTINCT trouvé en creusant la correction ci-dessus, corrigé
  (`5365b643`, à l'époque PAS déployé -- consigne opérateur explicite "ne deploie pas" ce
  segment ; **confirmé déployé depuis, ancêtre du HEAD en prod au 20/07** -- balayé par
  un déploiement groupé ultérieur, pas une décision de repousser cette consigne)** :
  `portfolio_summary()` ne lit `realized_pnl_partial` QUE pour les
...
- **#95 (coupe-circuit adaptatif par fournisseur, cascade OHLCV) -- évalué, construit,
  testé, à l'époque pas déployé (**confirmé déployé depuis, commit `63bbd7e7` ancêtre du
  HEAD en prod au 20/07** -- balayé par un déploiement groupé ultérieur).** Root cause
  du besoin : `_fetch_candles` retente TOUJOURS
...
**Contrainte matérielle VPS — 2 cœurs / ~1,8 Go RAM (vérifié 20/07 : `nproc`, `free -h`, charge observée 2,74 sur 2 cœurs, 810 Mo de swap déjà utilisés pendant une suite de tests + un déploiement concurrents).** Machine modeste, pas de marge — dispatcher/planifier le travail EN CONSÉQUENCE plutôt que de laisser plusieurs charges lourdes tourner en même temps sans y penser :
