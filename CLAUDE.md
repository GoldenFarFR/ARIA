...
- #420 (01/09, verified real) — Zerion hack: AI social engineering compromised a team member's session/credentials, not the agent. New mandate #192 case: operator-layer, not agent-reasoning.
...
- #422 (01/09) — Pools.trade/Memecoin.Fun now compete with Pons on Robinhood Chain. `robinhood_pump_shadow` sources via DexPaprika chain-wide trending pools (likely launchpad-agnostic already) — unconfirmed empirically this session (DB query blocked); next session: check `dex_id` diversity.
- #423 (01/09, verified WebSearch+WebFetch real, operator-flagged via a live GMGN screenshot) — InsightX ("Atlas"), a wallet-funding-graph tool covering Base+7 chains, directly targets `bundle_launch.py`'s own documented structural limit (distinguishing a real bundled snipe from independent same-block snipers). Same blocker as Bubblemaps (#295): no public API pricing, commercial contact required — banked, no code before a real price is obtained.
