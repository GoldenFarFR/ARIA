...
- #389 (operator research, 25/08, verified `gh api` + README) — `thewaltero/mythos-sentinel`: EIP-712 signed spend mandates + anti-reset local ledger + EAS on-chain attestation, coherent reference pattern for hardening agent-wallet pilots, overlaps #378. NOT ready to integrate (young/stale 2.5mo/unaudited, parent repo's "leaked Anthropic protocol" tagline is false marketing). Banked as design reference only. (full detail: `docs/backlog-technique.md`)
