...
**Tout déploiement VPS est exécuté par l'opérateur lui-même (consigne opérateur explicite, 16/07)** : Claude Code (cloud ou VPS) donne les commandes exactes (`git checkout main && git pull origin main && ./vanguard/deploy.sh` etc.) et vérifie le résultat, mais ne lance jamais lui-même la commande de déploiement à la place de l'opérateur.
