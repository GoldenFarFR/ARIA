...
- #394 (Avocat du Diable report, verified 28/08) — 5 duplicated pause modules, divergent fail-open/fail-closed semantics, 2 real incidents already confirm the risk class; `pause_registry.py` proposed, needs explicit operator go before any step (touches `outgoing_pause.py`, "do not recode"). (full detail: `docs/backlog-technique.md`)
- #395 (research-log promotion 28/08, verified WebSearch: arxiv.org 2608.10218, real) — "Mind Viruses": self-propagating instructions can spread through persistent, agent-edited system-prompt files across sessions; a cheap warning paragraph gives near-total immunity. CLAUDE.md is exactly this kind of file — future edit worth operator validation, not done here. (full detail: `docs/backlog-technique.md`)
- #396 (research-log promotion 28/08, verified WebSearch: arxiv.org 2608.05108, real) — PIMiner: transferable prompt-injection strategy library, 42.9% success vs Claude-Sonnet-4.5 (first quantified external figure against ARIA's own model family); WARD companion defense not independently re-verified. Feeds mandate #192. (full detail: `docs/backlog-technique.md`)
- #397 (research-log promotion 28/08, verified WebSearch: arxiv.org 2607.02795, real) — "Coordinated Sniper Cohorts on Pump.fun": 1012 wallet rings detected via rigorous clustering on ARIA's only active Solana sourcing venue. Candidate method for #379 (`smart_money.py` deposit clustering). (full detail: `docs/backlog-technique.md`)
- #398 (research-log promotion 28/08, verified WebSearch, real) — Step Finance ($40M treasury theft, 31/01/2026) shut down over compromised EXECUTIVE PERSONAL DEVICES, not a contract bug. New diligence criterion: device hygiene of anyone with real wallet access, not just server-side key custody. (full detail: `docs/backlog-technique.md`)
- #399 (research-log promotion 28/08, verified WebSearch: arxiv.org 2606.08232, real) — "Hour-Aware Adaptive Risk Management" (190-trade Solana paper run): top 1.6% of trades = 100% of profit, independently cross-confirming ARIA's own 20/08 corollary (1.8%/100%). First external same-asset-class paper-trading benchmark found. (full detail: `docs/backlog-technique.md`)
- #400 (research-log promotion 29/08, verified WebSearch, real) — Claude Code CLI/Agent SDK 3-CVE chain (2026-35020 TERM env command injection → malicious settings.json → 2026-35022 credential exfiltration), patched 2.1.92/0.1.56; the VPS harness's real version has never been checked against this specific CVE (attempted this pass, blocked by this session's own Bash approval gate). (full detail: `docs/backlog-technique.md`)
- #401 (research-log promotion 29/08, verified via code read) — `thesis_journal.py`/`conviction_research` have no structural requirement that every thesis claim link to a URL actually fetched (unlike MemeToro's launch-manifest validator) — confirmed gap against "raisonner uniquement sur des faits verifiables". (full detail: `docs/backlog-technique.md`)
- #402 (research-log promotion 29/08, verified WebSearch: Unit 42, real) — confirmed technique: a financial-advisory skill can pass an install-time scan clean and only fetch its malicious payload from an externally-hosted, later-rewritable page; no skill installed on the VPS today matches this profile, add the check for any future financial/analytical skill. (full detail: `docs/backlog-technique.md`)
- #403 (research-log promotion 29/08, verified WebSearch: Blockaid, real) — named real (not lab) exploits against production trading agents ($216k, Freysa/$47k via a single defeated NL instruction) — verify no ARIA guardrail (wallet_guard/pilot caps/kill-switch) is enforced only via a natural-language instruction rather than a structural code check. (full detail: `docs/backlog-technique.md`)
- #404 (research-log promotion 29/08, verified WebSearch: KuCoin report, real) — 45.6% of surveyed teams share API keys across agents (KuCoin); verify no non-wallet key (Chainstack/GoPlus/LLM) is shared between `agent_wallet_pilot` and low-privilege reading skills — this pass already confirmed the Anthropic client hits `api.anthropic.com` direct, no third-party router. (full detail: `docs/backlog-technique.md`)
- #405 (research-log promotion 29/08, verified WebSearch, real) — MetaMask "Agent Wallet" pairs agent trading with a real capped loss-insurance product ($10k/month) rather than prevention alone — reference idea only, no ARIA pilot compensates after the fact today. (full detail: `docs/backlog-technique.md`)
