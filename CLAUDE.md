...
- **Clarification forcée avant toute recherche exploratoire non triviale (25/08, inspiré de spec-kit's `/speckit-clarity`, demande opérateur explicite après avoir observé des recherches sans objectif clair : « des fois j'ai l'impression que tu fais des recherches juste pour le fun sans résultat »).** Avant de lancer une investigation qui dépasse une vérification ponctuelle (plusieurs outils, plusieurs pistes, un sujet nouveau), énoncer explicitement AVANT de commencer : ce que je comprends de l'objectif, et mes zones d'incertitude s'il y en a — jamais enchaîner les outils en silence sans que l'opérateur puisse voir si la recherche a un but clair. Ne s'applique pas aux vérifications ponctuelles courtes (un grep, un fait à confirmer) où ce serait du bruit.
