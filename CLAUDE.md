...
- #424 (research-log promotion 03/09, verified WebSearch: anthropics/scone-bench real) — Anthropic SCONE-bench: Claude Opus 4.5/GPT-5 already autonomously exploit smart contracts, 2 real zero-days found on post-knowledge-cutoff contracts. Two-sided reading (contract risk + candidate defensive capability for `safety_screen.py`): full diligence in `docs/aria-learning-inbox/2026-09-03-diligence-scone-bench-anthropic-exploit-capability.md`.
- #425 (research-log promotion 03/09, verified) — OWASP "Smart Contract Top 10" 2026 re-ranks Access Control to #1 and adds Proxy/Upgradeability as a new #10. Gap confirmed in the code (03/09): `blockscout.py` resolves a proxy's implementation ONLY ONCE at initial scan time (`implementation_address`), no re-scan exists if the logic changes AFTER ARIA's conviction (candidate GoPlus DeepScan, #330/#334, would remain the piece to wire to close this specific gap).
- #426 (research-log promotion 03/09, verified WebSearch: docs.birdeye.so/pricing real) — Birdeye (real-time Solana on-chain data, free tier 30k CU/1req-s) direct candidate for open system issue #278 (critical: persistent 402 Payment Required on DexPaprika for `solana_pump_shadow.py`) — test concretely (real quota, real Pump.fun coverage) before any switch.
- #427 (research-log promotion 03/09, verified WebSearch real) — Base "Creator Grant Program" (applications open since 02/09/2026, pure cash grants up to $4000, never a token) — external revenue angle for ARIA's public editorial strategy (`docs/playbook-editorial-aria.md`), no application filed without operator validation.
- #428 (research-log promotion 03/09, verified WebSearch real) — Robinhood Chain's free-gas subsidy ends ENTIRELY on 2026-09-29 (only a 90% reduction was dated so far, mid-August, #388) — direct calendar factor to verify concretely before budgeting real gas cost for the future Robinhood 2$ agent-wallet pilot.
- #429 (research-log promotion 03/09, verified WebSearch real: Pump.fun native TP/SL orders live since 2026-09-01) — check the most recent `solana_late_bonding_shadow` closures for artificial exit clustering at round thresholds (e.g. +200%/-50%) appearing since that date, an uncontrolled noise factor for `pocket_entry_sweep`.
- #430 (research-log promotion 03/09, verified WebSearch real) — MadeOnSol, first candidate found covering Solana AND Robinhood Chain SIMULTANEOUSLY (2075+ KOL wallets, Pump.fun deployer intel) for the already-banked lead #379 (Sybil clustering/diversification for `smart_money.py`) — compare concretely against Fomo (#419)/RepScore (#361) before any integration.
- #431 (research-log promotion 03/09, verified WebSearch: Socket.dev "Superior" campaign real, 08/2026) — 19 legitimate Chrome/Edge extensions bought out then compromised via a normal update (multi-chain wallet drainer). New test case for the permanent prompt-injection mandate #192: verify no real wallet access ever transits through a browser carrying unaudited extensions, including the `claude-in-chrome` fallback already used in cascade (cf. "browser fallback" memory).
