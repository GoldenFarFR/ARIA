...
**31/08 addition — ARIA Trader Assistant**: a PARALLEL app (never merged into ARIA Core, deliberate operator decision to avoid mixing with the just-closed Brique 6 on-chain audit) — a multi-dimension Telegram radar (social/narrative, chart trajectory, on-chain momentum, market-relative resilience, entry quality, narrative-cluster fit) enriched by FOMO (trade theses + PnL + trader balance, live on Robinhood Chain since July 2026) and X, never a single aggregate score, never BUY/SELL (only WATCH/ALERT/IGNORE, thresholds to calibrate later via replay). Goal explicitly reframed by the operator: not "predict the next x1000", but give a human trader faster/more complete/less emotional information than the standard UI — an edge matching top memecoin traders, never autonomous execution. Full architecture, the 6 scoring dimensions, the 8-tool suite (Radar/Narrative Engine/Chart Intelligence/Entry Quality/Market Relative/FOMO Intelligence/Exhaustion Engine/Position Assistant), and the "5 questions every alert must answer": `docs/vision-aria-trader-assistant.md`. Nothing decided yet on target repo/first prototype/timeline — resume only on the operator's next concrete "go".
...
- #420 (research-log promotion 01/09, verified WebSearch: crypto.news/MEXC — real, Zerion disclosed ~$100k hot-wallet loss, April 2026, Lazarus-linked) — AI-assisted social engineering compromised team members' logged-in sessions/credentials, not the agent's reasoning. New test case for mandate #192: verify no Claude Code/Anthropic API credential used by ARIA sessions could be extracted via social engineering targeting the OPERATOR (distinct from wallet_guard, which assumes the agent itself is honest but doesn't cover the session that pilots it being compromised).
- #421 (research-log promotion 01/09, verified WebSearch: Chainalysis/CryptoNexa/TechTimes — real, H1 2026 report) — Lazarus/TraderTraitor caused 55% ($609M) of all H1 2026 crypto hack losses, concentrated in a single 17-day April window (KelpDAO $292M + Drift, already logged #376/#410). Reinforces the existing diligence criterion for any future dormant-capital-on-Base deployment: catastrophic DeFi loss risk is concentrated in short state-actor attack windows, not diffuse — govern/shared-dependency checks matter more than a calm incident history.
- #422 (research-log promotion 01/09) — Robinhood Chain launchpad competition intensifying: Uniswap's "Pools.trade" and Memecoin.Fun (3.5M$ raised) now compete with Pons (#413). `robinhood_pump_shadow`/`_v2` source via DexPaprika's chain-wide trending-pools feed (launchpad-agnostic by design, not a per-launchpad allowlist), so this is likely already covered structurally — but not empirically confirmed this session (a `dex_id` distribution query on the live DB was blocked by session permissions). Quick follow-up for a future session: `SELECT dex_id, COUNT(*) FROM robinhood_pump_signals GROUP BY dex_id` and confirm launchpads beyond Pons actually appear.
