...
- **Ne jamais inspecter / grep / afficher / dumper une source portant des secrets (`.env`, `docker inspect`, `printenv`, `/proc/*/environ`) sauf si la tâche exige de modifier le secret lui-même (02/09, gravé après une fuite réelle — cette règle ne vivait que dans une mémoire de session, elle a cédé 3 fois).** Préférer les métadonnées : nom de variable, `configured=true`, empreinte sha256 tronquée, test de présence. Une blacklist de commandes ne suffit pas (`python -c`, `strings`, `docker inspect` la contournent) — la règle porte sur l'intention de lecture, jamais sur l'outil. Mesurer ce qui reste réellement atteignable : `python3 scripts/secret-exposure-audit.py` (empreintes seulement, jamais une valeur). Détail : `docs/HANDOFF_SECURITE.md`.
...
**31/08 addition — ARIA Trader Assistant**: a PARALLEL app (never merged into ARIA Core, deliberate operator decision to avoid mixing with the just-closed Brique 6 on-chain audit) — a multi-dimension Telegram radar (social/narrative, chart trajectory, on-chain momentum, market-relative resilience, entry quality, narrative-cluster fit) enriched by FOMO and X, never a single aggregate score, never BUY/SELL (only WATCH/ALERT/IGNORE, thresholds calibrated later via replay). Goal explicitly reframed by the operator: not "predict the next x1000", but give a human trader faster/more complete/less emotional information than the standard UI — never autonomous execution. Full architecture, the 6 scoring dimensions, the 8-tool suite and the "5 questions every alert must answer": `docs/vision-aria-trader-assistant.md`. Nothing decided yet on target repo/first prototype/timeline — resume only on the operator's next concrete "go".
...
**Deployment cadence — governed by the Zero-Permission Policy above: deploy D'INITIATIVE as soon as the suite is green, never asking.** One surviving nuance (a real cost, not a permission question): a doc-only change or a fast iteration on the SAME subsystem can ride the next deployment rather than forcing a Docker rebuild per micro-tweak (18/07: 3 deployments for tweaks that fit in 1). Anything touching runtime behavior, security, or a live capability deploys immediately. After deploying, ALWAYS update `.claude/last-deployed-ref` (`git rev-parse main`) and commit it — that resets the undeployed-lines counter.
...
Docker backend `aria-api`, binding **strictly `127.0.0.1:8000/8001`** (blue-green alternation, NEVER public), nginx as the front (TLS) via a dedicated upstream (`/etc/nginx/conf.d/aria-api-upstream.conf`, outside the repo). Data bind-mount `/opt/aria-data`; **configuration arrives through a read-only MOUNT parsed by `vanguard/docker-entrypoint.py`, never `--env-file`** (02/09 — `--env-file` copied every secret into `docker inspect`; cf. the secrets rule above). `vanguard/deploy.sh` (build + health check). **Near-instant rollback (#154/#157, 13/07)**: the new container is launched and checked WHILE the old one still serves, so a failed health-check causes ZERO downtime; same dual-criterion protection on the static showcase (`vanguard/deploy-vitrine.sh`). Plus `willfarrell/autoheal` and a homemade circuit breaker (`vanguard/scripts/autoheal-circuit-breaker.sh`, 3 restarts/10 min max). Full detail: `docs/deploy-rollback-blue-green.md`. **VPS access, IP, and infra: private, in `aria-ops`.** Priority security: SSH key-only + fail2ban + firewall.
...
- #420 (01/09) — Zerion hack: AI social engineering compromised a team member's session, not the agent. Mandate #192 case: operator-layer, not agent-reasoning.
- #421 (01/09, verified real) — Chainalysis H1 2026: Lazarus = 55%/$609M of losses in one 17-day window. Reinforces #376/#410's diligence criterion (concentration risk > calm history) for any Base dormant-capital deployment.
- #422 (01/09) — Pools.trade/Memecoin.Fun now compete with Pons on Robinhood Chain; `robinhood_pump_shadow` sources chain-wide (likely launchpad-agnostic) — to confirm: `dex_id` diversity.
- #423 (01/09) — InsightX "Atlas" (wallet funding-graph) vise le trou documente de `bundle_launch.py`. Free API reelle. Ouvert : Atlas est-il dans le tier gratuit ?
- #424 (research-log promotion 03/09, verified WebSearch: anthropics/scone-bench réel) — Anthropic SCONE-bench : Claude Opus 4.5/GPT-5 exploitent déjà en autonomie des smart contracts, 2 zero-days réels trouvés sur des contrats post-coupure de connaissance. Double lecture (risque contractuel + capacité défensive candidate pour `safety_screen.py`) : diligence complète `docs/aria-learning-inbox/2026-09-03-diligence-scone-bench-anthropic-exploit-capability.md`.
- #425 (research-log promotion 03/09, verified) — OWASP "Smart Contract Top 10" 2026 reclasse Access Control #1 et fait entrer Proxy/Upgradeability en #10 (nouveau). Gap confirmé dans le code (03/09) : `blockscout.py` résout l'implémentation d'un proxy UNE SEULE FOIS au moment du scan initial (`implementation_address`), aucun re-scan n'existe si la logique change APRÈS la conviction d'ARIA (le candidat GoPlus DeepScan, #330/#334, resterait la brique à câbler pour combler ce point précis).
- #426 (research-log promotion 03/09, verified WebSearch: docs.birdeye.so/pricing réel) — Birdeye (données on-chain Solana temps réel, palier gratuit 30k CU/1req-s) candidat direct pour l'issue système #278 (critique, ouverte : 402 Payment Required persistants sur DexPaprika pour `solana_pump_shadow.py`) — à tester concrètement (vrai quota, vraie couverture Pump.fun) avant toute bascule.
- #427 (research-log promotion 03/09, verified WebSearch réel) — Base "Creator Grant Program" (candidatures ouvertes depuis le 02/09/2026, subventions cash pures jusqu'à 4000$, jamais de token) — angle de revenu externe pour la stratégie éditoriale publique d'ARIA (`docs/playbook-editorial-aria.md`), aucune candidature déposée sans validation opérateur.
- #428 (research-log promotion 03/09, verified WebSearch réel) — la subvention de gas gratuit sur Robinhood Chain se termine ENTIÈREMENT le 29/09/2026 (jusqu'ici seule une réduction de 90% mi-août était datée, #388) — facteur de calendrier direct à vérifier concrètement avant de budgéter le coût de gas du futur pilote agent-wallet Robinhood 2$.
- #429 (research-log promotion 03/09, verified WebSearch réel : ordres natifs TP/SL Pump.fun live depuis le 01/09/2026) — vérifier sur les clôtures les plus récentes de `solana_late_bonding_shadow` si des paliers de sortie artificiels à des seuils ronds (ex. +200%/-50%) apparaissent depuis cette date, facteur de bruit non contrôlé dans `pocket_entry_sweep`.
- #430 (research-log promotion 03/09, verified WebSearch réel) — MadeOnSol, premier candidat trouvé couvrant SIMULTANÉMENT Solana ET Robinhood Chain (2075+ wallets KOL, deployer intel Pump.fun) pour la piste déjà banquée #379 (clustering Sybil/diversification `smart_money.py`) — à comparer concrètement contre Fomo (#419)/RepScore (#361) avant toute intégration.
- #431 (research-log promotion 03/09, verified WebSearch: campagne Socket.dev "Superior" réelle, 08/2026) — 19 extensions Chrome/Edge légitimes rachetées puis compromises via mise à jour normale (wallet-drainer multi-chain). Nouveau point de test pour le mandat prompt-injection #192 : vérifier qu'aucun accès wallet réel ne transite par un navigateur muni d'extensions non auditées, y compris l'extension de secours `claude-in-chrome` déjà utilisée en cascade (cf. mémoire "browser fallback").

## Required reading (the detailed brain)
**Recherche alpha : `docs/mandat-recherche-alpha.md`** (mandat opérateur verbatim, 02/09, 20 règles) + `docs/hypotheses-registre.md` (statut de chaque hypothèse).
