...
- **Audit dexpulse/Aria Market (08/07 nuit) — cartographié, PAS ENCORE nettoyé** : l'opérateur veut purger les noms de produit obsolètes "dexpulse"/"Aria Market". Audit fait (79 fichiers réels), aucun changement appliqué. **Bug réel trouvé en chemin** : le heartbeat re-sème "Aria Market" comme filiale active dans la base à chaque cycle (`repertoire_skill.execute_develop_repertoire`), contredisant les faits établis ("aucune filiale active") — à corriger indépendamment du nom choisi. Le nettoyage touche un vrai fichier VPS (`dexpulse.db`), un cookie de session déjà posé chez les membres, et **plusieurs endroits qui gardent le nom exprès** (garde-fous anti-hallucination détectant les affirmations obsolètes) — pas un simple find/replace. Voir `docs/etat-systeme-cable.md` pour le détail avant d'y toucher.
