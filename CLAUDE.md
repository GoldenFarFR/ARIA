...
- **GoPlus Security X watch (18/08, operator request)**: `/opt/aria-data/goplus-security-watch/run.sh`, VPS cron daily (1 call/day). Fetches @GoPlusSecurity's tweets via the REAL x402 twit.sh API (`services/twitsh.py`, $0.01/call, shared `x402_budget.py` cap) -- RAW verbatim text, never WebFetch/browser-summarized (operator's explicit requirement, confirmed live that WebFetch/browser both translate+truncate). Dedupes by tweet_id AND restricted to the last 24h. The existing Research Watch (`research-loop/prompt.txt`) reads the resulting log as an extra source -- itself never makes the paid call. Detail: `docs/HANDOFF_AUTOMATISATION.md`.
