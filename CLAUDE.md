...
- **GoPlus Security X watch (18/08, operator request, extended same day)**: `/opt/aria-data/goplus-security-watch/run.sh`, VPS cron daily. 3 chained stages: (1) fetches @GoPlusSecurity's tweets via the REAL x402 twit.sh API (`services/twitsh.py`, $0.01/call) -- RAW verbatim text, never WebFetch/browser-summarized (confirmed live both translate+truncate), dedupes by tweet_id, restricted to the last 24h; (2) a second restricted headless `claude -p` pass (same hard tool limits as `research-loop`) judges EACH tweet -- explicit TRAITER/NON verdict + a concrete text-only proposed solution when TRAITER, never code; (3) `promote_verdicts.py` (deterministic, no LLM) opens a `system_issues` entry per TRAITER verdict AND sends a direct Telegram notification (operator: "je réouvre des sessions très rarement" -- `system_issues`'s session-start surfacing alone isn't enough, added same day). The existing Research Watch also reads the raw-tweet log as an extra source, itself never making the paid call. **Automation ceiling found the same day**: an attempt to build a further autonomous stage (implement the TRAITER proposal + close the issue, no human present) was blocked by the safety classifier 3 times running, including on a purely deterministic close+notify script with zero LLM call -- confirms the boundary is "no human present at the moment of action", not specifically "public push" (`scripts/research-log-promotion.sh`'s own autonomous commit+push already works, but only because a Claude Code session is nominally "present" as the actor). Actual implementation of a TRAITER verdict stays a task for a real session to pick up from `system_issues`/Telegram, never a further unsupervised stage. Detail: `docs/HANDOFF_AUTOMATISATION.md`.
