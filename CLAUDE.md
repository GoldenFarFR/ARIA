...
- **Après tout compactage de mémoire ou démarrage de session, lire `docs/etat-systeme-cable.md` avant toute inspection de code (26/08, gravé après une session qui a dû ré-investiguer un câblage RPC déjà connu juste après un compactage).** Ce fichier existe précisément pour éviter de reposer/re-creuser des questions déjà répondues — le lire en premier, jamais après coup.
- **Toute nouvelle skill créée dans ce projet porte `disable-model-invocation: false` par défaut (26/08, décision opérateur explicite, appliquée rétroactivement aux 7 skills speckit).** N'y déroger que pour une raison explicite propre à cette skill précise (jamais par défaut/prudence générique).
...
### Routeur spec-kit / Fast-Track (25/08)
**Fast-Track** (pas de `specs/`, on code directement) SI les 3 conditions sont réunies : aucun garde-fou ni capital réel touché ; aucun paramètre de stratégie modifié (seuil, stop, filtre, borne) ; couvert par les tests existants. **Un hotfix prod est TOUJOURS Fast-Track** quelle que soit sa taille (sinon contradiction avec le mandat d'auto-guérison). **Tout le reste → cycle spec-kit complet.** Critère = le RISQUE, jamais le nombre de lignes (une ligne sur un plafond de trading > 200 lignes de tests). Un spec-kit obligatoire à 100% est explicitement refusé : il multiplierait les compactages et aggraverait l'amnésie qu'il corrige.
...
- **CabalSpy sourcing: read-only KOL directory only, `gate ARIA_CABALSPY_SOURCING_ENABLED` — VERIFY the real state** — its former link into the wallet-scoring pipeline (`/walletscore` candidate sourcing) was removed 25/08 along with the entire wallet-scoring mechanism (operator decision); it now only feeds `skills/cabalspy_candidate_sourcing.py`'s categorization track, never a trading signal.
...
- #382 (research-log promotion 25/08, verified WebSearch: credprotocol.com) — Cred Protocol ships a dedicated Sybil Detection endpoint (funding-source clustering + transaction-timing entropy + identity attestations), a concrete method candidate for #379's deposit-clustering extension to `smart_money.py`. Pricing/Base-Solana coverage still unverified. (full detail: `docs/backlog-technique.md`)
- #383 (research-log promotion 25/08, verified WebSearch: real incident, May 2026, ~150-200k$ DRB stolen) — Bankrbot/Grok drained via a Morse-code-encoded prompt injection that slipped past a plain-text safety filter, after a gifted NFT silently escalated the agent's transfer privileges. Concrete case for mandate #192: verify no text-reading skill (tweets/DMs/docs) can reach `agent_wallet_pilot` with an encoded instruction. (full detail: `docs/backlog-technique.md`)
- #384 (research-log promotion 25/08, verified WebSearch + code check) — ARIA's real Solana pilot (`services/jupiter.py`) still quotes via the legacy `lite-api.jup.ag/swap/v1/quote`, while Jupiter's Ultra v3 (34x better sandwich protection, gasless, lower fees) is already live. Evaluate migrating the real-capital swap path before any pilot scale-up — directly tied to the carved-in-stone slippage/MEV rule. (full detail: `docs/backlog-technique.md`)
- #385 (research-log promotion 25/08, verified WebSearch: real, 07/08+13/08/2026) — Pump.fun's "Callouts" feature (public token shill broadcast to followers, +44% callouts/+87% replies) is a new native social pre-pump signal on the one active Solana sourcing pocket's launchpad. Verify whether `signal_cascade_x`/`_web` already captures it or it's a blind spot. (full detail: `docs/backlog-technique.md`)
- #386 (research-log promotion 25/08, verified WebSearch: real, $8.5M+ face-value, 21-22/08/2026) — The Sandbox's SAND bridge was drained via a LayerZero delegate-permission hijack (`approveAndCall`), a privilege-escalation vector distinct from #376's governance-capture pattern. Reinforces the 17/08 CCIP-over-LayerZero choice — add "delegate/permission model audit" as an explicit CCIP diligence line before any real dormant-capital bridging. (full detail: `docs/backlog-technique.md`)
- #387 (research-log promotion 25/08, verified WebSearch: real, $370-500M extracted/16 months) — Confirms the existing slippage≤10% rule empirically and adds a candidate hardening layer (on-chain TWAP-deviation circuit breaker, stacked defenses beat a single one) for the real Solana pilot. Evaluate only if the pilot scales beyond current volume. (full detail: `docs/backlog-technique.md`)
- #388 (research-log promotion 25/08, verified WebSearch: real, mainnet since 01/07/2026) — Robinhood Chain now has two live public AMMs (Uniswap v2/v3/v4/UniswapX since mainnet launch, Pleiades since day one) — updates the stale 23/08 "no swap mechanism yet" note in the Robinhood pilot section above. Full diligence before continuing the in-isolation router build: `docs/aria-learning-inbox/2026-08-25-diligence-robinhood-chain-amm-live.md`.
- #389 (operator research, 25/08, verified via source code read, not stars) — `thewaltero/mythos-sentinel`: EIP-712 signed spend mandates + anti-reset local ledger + EAS on-chain attestation. Real engineering quality confirmed (clean modular code, real test suite + green CI, single dependency) — solid reference for hardening agent-wallet pilots, overlaps #378. Not wired in: unmaintained 2.5mo, pre-1.0, unaudited. (full detail: `docs/backlog-technique.md`)
- #390 (research-log promotion 26/08, verified WebSearch: real, patched Aug 2026) — Ledger Ethereum app clear-signing race condition (WebHID tx substitution during the human review window), found by TestMachine's autonomous scanner Azimuth (benchmarked on EVMBench). Concrete test case for #369 (ERC-7730) + Azimuth/EVMBench as candidate audit tools for the future homemade wallet, testnet-only today. (full detail: `docs/backlog-technique.md`)
- #391 (research-log promotion 26/08, verified WebSearch: arxiv.org 2601.22185, real) — MemeChain: open dataset, 34,988 cross-chain memecoins (Ethereum/BSC/Solana/Base) with on-chain + off-chain (site HTML, logo, socials) artifacts, 5.15% stop trading within 24h. First external, labelled ground truth found to calibrate ARIA's legitimacy engine against, per the 22/08 doctrine that a system can't validate its own verdicts. (full detail: `docs/backlog-technique.md`)
- #392 (research-log promotion 26/08, verified WebSearch: zscaler.com/securityweek.com, real, ~07/2026) — Zscaler ThreatLabz documents 2 live indirect-prompt-injection campaigns against AI agents: CSS-hidden payment instructions in SEO-poisoned fake API docs, and a DeBank typosquat (debank[.]auction) that fooled Claude Sonnet 4.5 + GPT-5.4 into treating it as legitimate. Concrete new test cases for the standing prompt-injection mandate #192. (full detail: `docs/backlog-technique.md`)
- #393 (research-log promotion 26/08, verified WebSearch: proof.com/helpnetsecurity.com, real) — x401 (Proof): open agent-identity protocol backed by Circle/OpenAI/Google/Okta, FIDO Alliance sponsor since 05/2026, explicitly complementary to x402 (identity layer vs payment layer). Relevant to ARIA's dormant x402 seller channel (issue #245) if agentic buyers start requiring identity proof before paying — no action before real adoption is measurable. (full detail: `docs/backlog-technique.md`)
