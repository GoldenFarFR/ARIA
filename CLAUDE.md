...
Backend Docker `aria-api`, binding **strictement `127.0.0.1:8000`** (JAMAIS public), nginx en façade (TLS). Data bind-mount `/opt/aria-data`. `vanguard/deploy.sh` (build + health check). **Rollback PAS automatique (corrigé 11/07, vérifié par VPS Secondaire)** : en cas d'échec du health check, le script tague l'ancienne image `aria-api:rollback` et s'arrête (`exit 1`) — il ne relance pas ce conteneur tout seul, il faut le faire à la main. Vitrine : `vanguard/deploy-vitrine.sh`. **Accès VPS, IP et infra : privés, dans `aria-ops`.** Sécu prioritaire : SSH clé-only + fail2ban + firewall (l'IP a fuité dans l'historique public → durcir SSH est le vrai correctif).
