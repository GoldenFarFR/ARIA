...
## Mise à jour 17/07 — correction : cette session A un accès VPS réel, contrairement à ce que plusieurs entrées ci-dessus supposaient
Vérifié concrètement (`docker ps`, `pwd`, accès à `/opt/aria-data/aria.db`, `curl 127.0.0.1:8000`) : cette session tourne bien sur le VPS de prod (`/opt/aria`) — accès réseau réel, Docker, filesystem prod. Ça correspond exactement à la ligne « Claude Code installé DIRECTEMENT sur le VPS » (08/07, section Capacités), jamais recontrôlée depuis. Plusieurs entrées entre le 11/07 et le 16/07 (dispatch VPS Principal/Secondaire/Research pour tout ce qui touchait au réseau, endpoints diagnostic #158/#159 pensés comme contournement, « cette session cloud n'a pas d'accès réseau direct au VPS ») reposaient sur cette limite jamais revérifiée après le constat initial du 08/07 — ne pas les prendre pour argent comptant, revérifier à chaque nouvelle session plutôt que de supposer que ça vaut encore.

**Conséquence pratique actée le 17/07** : déploiement (`./vanguard/deploy.sh`) désormais exécutable DIRECTEMENT par une session confirmant un accès VPS réel comme celle-ci, sur demande opérateur explicite — la règle « l'opérateur déploie toujours lui-même » (16/07, section Déploiement) est assouplie en conséquence, précisée sur place. Déploiement du 17/07 exécuté et vérifié ainsi (health check confirmé, commit `9db4505f`).
...
**Question ouverte pour l'opérateur, pas tranchée unilatéralement** : le dispatch systématique vers des sessions VPS séparées (Principal/Secondaire/Research) reste-t-il nécessaire pour la parallélisation, ou cette session peut-elle directement effectuer elle-même une partie de ce travail désormais (déploiement, vérification `aria.db`, diagnostics) ? Ne pas changer l'organisation du travail sans le redemander.
