...
- [X] T019 Ask the operator to add `ARIA_LIVE_SIGNAL_OBSERVER_ENABLED=true` (and optionally `ARIA_SIGNAL_TELEGRAM_CHAT_ID`) to `vanguard/backend/.env` (never touched by this session), then redeploy; verify `docker inspect aria-api` shows the gate and logs show `live_signal_observer: started (4 endpoints)`; run quickstart.md Scenario 1 (SC-001/SC-002).
