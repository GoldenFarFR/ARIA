...
- [ ] T003 [P1] IN PROGRESS 25/08 -- instrumented, not yet measured: `PumpSwapWebSocketFeed` gained `notification_count`/`pool_count` properties (real accountNotification counter, never guessed), logged every 300s by `shadow_persistent.py` (`PUMPSWAP shadow: pools_tracked=X notifications_total=Y`) while still on Helius. 2 new tests. **Needs a real observation window (a few hours) before the real push rate can be read and compared to the 175k/day Solana cap.**
- [ ] T004 [P1] IN PROGRESS 25/08, same pattern -- `PumpFunBondingWebSocketFeed` gained the same two properties (`notification_count` sums the already-tracked `_trades_total` per-pool counters, no new state), logged every 300s (`PUMPFUN BONDING shadow: ...`). 3 new tests. Same real-data wait as T003.
