...
- Timeframe/lookback sweep (4H/15M candles, 24h/48h/6h lookback windows): launched 25/08, result retrieved -- **inconclusive, not a finding**. The 4H variants returned `avg_all` in the 1e31 range (a clear data/computation artifact, not a real PnL figure -- likely one corrupted/near-zero candle price exploding a multiplier on a population of only ~90 distinct tokens, small enough that one bad ratio dominates even the no-top5 trim). The 15M variants only matched 1 closure each (24h/6h lookback) -- far too small to read anything into. Conclusion: this scratch harness needs a sanity clamp on per-trade multiplier (reject/flag any single closure whose multiplier is off by orders of magnitude) before it can be trusted -- not pursued further this session, deprioritized after the pivot to Robinhood Chain (specs/004-shadow-robinhood).
