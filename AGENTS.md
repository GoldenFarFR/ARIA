...
- **Dune Analytics** (`services/dune.py`, cf. `docs/dune-integration-plan.md`) : client Execute SQL (dôme habituel, clé `DUNE_API_KEY` optionnelle lue par appel, `available=False` immédiat sans clé — jamais d'appel réseau). `run_sql_and_wait` orchestre exécution + sondage de statut (gratuit côté Dune) + lecture du résultat, borné (`max_wait`, jamais une attente non bornée). `build_early_buyer_multiple_query` (§3.2) et `build_recent_base_pairs_query` (§134) sur `dex.trades` — **schéma ET les deux requêtes composées vérifiés en exécution réelle via le MCP Dune** (§8 du plan) ; un bug réel (`peak_multiple` aberrant, dust trade) trouvé et corrigé (#185, §9). `get_first_funded_by`/`build_addresses_stats_query` (15/07, nuit suivante, §10 du plan) : interroge `addresses.stats` pour une liste d'adresses et retourne `first_funded_by`/`first_funded_at`/`is_eoa`/`is_smart_contract` — en appui du signal de financement partagé déjà existant (`_pairwise_convergence`, `smart_money.py`). **Vérifié en direct** (un bug réel de type trouvé et corrigé avant merge : `address` est `varbinary` dans `addresses.stats`, pas `varchar` — les littéraux doivent être des hex nus `0x...`, jamais entre guillemets). Aucun gate `ARIA_DUNE_ENABLED`, aucune tâche heartbeat, aucun appel depuis `wallet_candidate_sourcing.py`/`smart_money.py` — décisions opérateur explicites, l'intégration réelle à ces pipelines reste une tâche séparée pour chacune de ces trois requêtes.
